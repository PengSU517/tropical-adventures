        ...
        timeline =
        {
            ...
            TimeEvent(34 * FRAMES, function(inst)
                if pegou == 3 then
                    ...
                    -- print(x, y, z)
                    local pond = ponds[tostring(inst.GUID)]
                    if pond and pond.components.fishable ~= nil then
                        pond.components.fishable.fishleft = pond.components.fishable.fishleft + 1
                        if pond.components.fishable:GetFishPercent() > 0 then
                            local fish = pond.components.fishable:HookFish(inst)
                            if fish ~= nil then
                                inst:PushEvent("fishingstrain")
                                local caughtfish = pond.components.fishable:RemoveFish(fish)
                                if caughtfish then
                                    if caughtfish.Physics ~= nil then
                                        caughtfish.Physics:SetActive(true)
                                        caughtfish.Physics:Teleport(inst.Transform:GetWorldPosition())
                                    else
                                        caughtfish.Transform:SetPosition(inst.Transform:GetWorldPosition())
                                    end
                                    inst:PushEvent("fishingcatch", { build = caughtfish.build })
                                    caughtfish.entity:Show()
                                    if caughtfish.DynamicShadow ~= nil then
                                        caughtfish.DynamicShadow:Enable(true)
                                    end
                                    caughtfish.persists = true
                                    inst:PushEvent("fishingcollect", { fish = caughtfish })
                                end
                            end
                        end
                    end
