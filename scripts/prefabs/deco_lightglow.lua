...
local lighttypes = {
    ...
    electric_1 = {
        day = { rad = 8, intensity = 0.9, falloff = 0.5, color = { 1 * MULT, 1 * MULT, 1 * MULT } },
    },
}
...
local phasefunctions =
{
    day = function(inst, instant)
        ...
        inst.components.lighttweener:StartTween(nil, lights.day.rad, lights.day.intensity, lights.day.falloff,
            { lights.day.color[1], lights.day.color[2], lights.day.color[3] }, time)
    end,
    ...
    dusk = function(inst, instant)
        ...
        inst.components.lighttweener:StartTween(nil, lights.dusk.rad, lights.dusk.intensity, lights.dusk.falloff,
            { lights.dusk.color[1], lights.dusk.color[2], lights.dusk.color[3] }, time)
    end,
    ...
    night = function(inst, instant)
        ...
        if TheWorld.state.moonphase == "full" then
            ...
            inst.components.lighttweener:StartTween(nil, lights.full.rad, lights.full.intensity, lights.full.falloff,
                { lights.full.color[1], lights.full.color[2], lights.full.color[3] }, time)
        else
...
local function setLightType(inst, lighttype)
    if lighttypes[lighttype] then
        ...
        inst.Light:SetColour(lighttypes[inst.lighttype].day.color[1], lighttypes[inst.lighttype].day.color[2],
            lighttypes[inst.lighttype].day.color[3])
