...
local function RoomSectionfn(prefabname, build, bank, animdata, data)
    ...
    local function placeAlign(inst)
        ...
        --------------调整在墙面上的高度
        if data.height then
            local change = TheInput:IsKeyDown(TUNING.tro_pairedkey[TA_CONFIG.CLIENT.build_height_key][2]) and data
                .changevalue or
                TheInput:IsKeyDown(TUNING.tro_pairedkey[TA_CONFIG.CLIENT.build_height_key][1]) and -1 * data.changevalue or
                0
            if change ~= 0 then
