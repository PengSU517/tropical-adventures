...
local function makefn(stage, artfn, product, dig_product, burnable, pick_sound, patch)
	local function fn(Sim)
		...
		-- local x, y, z = inst.Transform:GetWorldPosition()
		-- if inst.prefab ~= "grass_tallwater" then
		-- 	if true then
		-- 		local fx = SpawnPrefab("grasswater")
...
local function patchfn()
	...
	inst:DoTaskInTime(0, function()
		...
		for i = 1, math.random(5, 10) do --    for i=1,math.random(50,200) do
			placeitemoffgrids(x, z, 12, "grass_tall", { GROUND.PLAINS, GROUND.DEEPRAINFOREST, GROUND.RAINFOREST }, nil,
				inst)
		end
	end)
	return inst
...
local function patchasparagus()
	...
	inst:DoTaskInTime(0, function()
		...
		for i = 1, math.random(2, 6) do
			placeitemoffgrids(x, z, 2, "asparagus_planted", { GROUND.PLAINS, GROUND.DEEPRAINFOREST, GROUND.RAINFOREST },
				nil, inst)
		end
	end)
	return inst
...
return
	Prefab("forest/objects/grass_tall",
		makefn(0, makegrass, "cutgrass", "dug_grass", true, "dontstarve/wilson/pickup_reeds"), grassassets, grassprefabs),
	Prefab("forest/objects/grass_tallwater",
		makefn2(0, makegrass, "cutgrass", "dug_grass", true, "dontstarve/wilson/pickup_reeds"), grassassets, grassprefabs),
	Prefab("forest/objects/grass_tall_infested",
		makefn(0, makegrass, "cutgrass", "dug_grass", true, "dontstarve/wilson/pickup_reeds", true), grassassets,
		grassprefabs),
