...
local function setupnavs(inst)
        ...
        for i = 2, total do
            ...
            if i == 4 then
                ...
                local swim_offset = FindSwimmableOffset(origin, math.random() * PI * 2, dist, triesmax, true, nil, nil,
                    true)
                if swim_offset then
                    inst.components.knownlocations:RememberLocation("nav" .. i,
                        Vector3(origin.x + swim_offset.x, 0, origin.z + swim_offset.z))
                end
            end
        end
    ...
    if not inst.components.timer:TimerExists("lifetime") then
        inst.components.timer:StartTimer("lifetime",
            data.schoollifetimemin + (math.random() * (data.schoollifetimemax - data.schoollifetimemin)))
    end
end
