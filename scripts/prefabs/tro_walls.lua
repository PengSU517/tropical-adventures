...
local function onhealthchange(inst, old_percent, new_percent)
    ...
    if new_percent > 0 then
        ...
        if inst.prefab == "wall_enforcedlimestone" then
            inst.AnimState:PlayAnimation("water_" .. anim_to_play .. "_hit")
            inst.AnimState:PushAnimation("water_" .. anim_to_play, true)
        else
        ...
        if inst.prefab == "wall_enforcedlimestone" then
            inst.AnimState:PlayAnimation("water_" .. anim_to_play, true)
        else
...
function MakeWallType(data)
    ...
    local function fn()
        ...
        if data.name == "limestone" then
            inst.components.repairable.repairmaterial = MATERIALS.LIMESTONE
        end

        if data.name == "enforcedlimestone" then
            inst.components.repairable.repairmaterial = MATERIALS.ENFORCEDLIMESTONE
        end
    ...
    return Prefab("wall_" .. data.name, fn, assets, prefabs),
        Prefab("wall_" .. data.name .. "_item", itemfn, assets,
            { "wall_" .. data.name, "wall_" .. data.name .. "_item_placer" }),
        MakePlacer("wall_" .. data.name .. "_item_placer", "wall", "wall_" .. data.name, "half", false, false, true, nil,
            nil, "eight")
end
