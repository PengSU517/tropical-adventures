...
local function entrance()
	...
	if inst.entrada == nil then
		...
		if part ~= nil then
			part.Transform:SetPosition(x - 3.8, 0, z - 11 / 3.5)
		...
		inst:DoTaskInTime(0, function(inst)
			...
			if inst.prefab == "playerhouse_room_metal_cima" then
				local portaentrada = SpawnPrefab("playerhouse_city_door_metal_cima")
				local a, b, c = inst.Transform:GetWorldPosition()
				portaentrada.Transform:SetPosition(a, b, c)
				portaentrada.components.teleporter.targetTeleporter = inst.exit
				inst.exit.components.teleporter.targetTeleporter = portaentrada
			...
			if inst.prefab == "playerhouse_room_pano_cima" then
				local portaentrada = SpawnPrefab("playerhouse_city_door_pano_cima")
				local a, b, c = inst.Transform:GetWorldPosition()
				portaentrada.Transform:SetPosition(a, b, c)
				portaentrada.components.teleporter.targetTeleporter = inst.exit
				inst.exit.components.teleporter.targetTeleporter = portaentrada
			...
			if inst.prefab == "playerhouse_room_peagank_cima" then
				local portaentrada = SpawnPrefab("playerhouse_city_door_peagank_cima")
				local a, b, c = inst.Transform:GetWorldPosition()
				portaentrada.Transform:SetPosition(a, b, c)
				portaentrada.components.teleporter.targetTeleporter = inst.exit
				inst.exit.components.teleporter.targetTeleporter = portaentrada
...
local function fnescada1()
	...
	inst.AnimState:PlayAnimation("pedra")
...
local function fnescada2()
	...
	inst.AnimState:SetBank("portas")
	inst.AnimState:SetBuild("portas")
	inst.AnimState:PlayAnimation("metal")
	inst.Transform:SetScale(2, 2, 2)
	--    inst.AnimState:SetLayer(LAYER_WORLD_BACKGROUND)
...
local function fnescada3()
	...
	inst.AnimState:SetBank("portas")
	inst.AnimState:SetBuild("portas")
	inst.AnimState:PlayAnimation("pano")
	--    inst.AnimState:SetOrientation(ANIM_ORIENTATION.OnGround)
	inst.Transform:SetScale(2, 2, 2)
	--    inst.AnimState:SetLayer(LAYER_WORLD_BACKGROUND)
...
local function fnescada4()
	...
	inst.AnimState:SetBank("portas")
	inst.AnimState:SetBuild("portas")
	inst.AnimState:PlayAnimation("peagank")
	--    inst.AnimState:SetOrientation(ANIM_ORIENTATION.OnGround)
	--    inst.AnimState:SetLayer(LAYER_WORLD_BACKGROUND)
	inst.Transform:SetScale(2, 2, 2)
...
local function placeTestFn(inst, pt)
	...
	local ents = TheSim:FindEntities(pt.x, pt.y, pt.z, 2, nil, { 'quadro' })
...
function MakePlacerporta(name, bank, build, anim, onground, snap, metersnap, scale, fixedcameraoffset, facing,
	...
	local function fn()
		...
		inst.components.placer.can_build = placeTestFn
...
return
	Prefab("playerhouse_room_pedra_cima", entrance),
	Prefab("playerhouse_room_metal_cima", entrance),
	Prefab("playerhouse_room_pano_cima", entrance),
	Prefab("playerhouse_room_peagank_cima", entrance),
	Prefab("playerhouse_city_door_pedra_cima", fnescada1, assets),
