...
{
    normal = {
        file = "tree_rainforest_webbed_build",
...
local function IsDefender(child)
    return child.prefab == "spider_ape"
end
...
local function SpawnDefenders(inst, attacker)
    ...
    if inst.components.childspawner ~= nil then
        ...
        local num_to_release = math.min(max_release_per_stage[inst.stage] or 1,
            inst.components.childspawner.childreninside)
        ...
        for k = 1, num_to_release do
            inst.components.childspawner.childname = k <= num_warriors and "spider_ape" or "spider_ape"
...
local function makefn(build, stage, data)
    local function fn(Sim)
        ...
        inst.components.childspawner.emergencychildname = "spider_ape"
...
return tree("spider_ape_tree", "normal", 0),
    tree("spider_ape_tree_normal", "normal", 2),
    tree("spider_ape_tree_tall", "normal", 3),
    tree("spider_ape_tree_short", "normal", 1),
    tree("spider_ape_tree_burnt", "normal", 0, "burnt"),
    tree("spider_ape_tree_stump", "normal", 0, "stump")
