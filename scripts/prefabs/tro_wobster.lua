...
local function on_ground_lobster_landed(inst)
    if inst.components.inventoryitem:IsHeld() then
        return
    end
...
local function base_land_wobster(build_name, nameoverride, fish_def, fadeout, cook_product)
    ...
    inst.AnimState:SetBank("wobster")
    inst.AnimState:SetBuild(build_name)
    inst.AnimState:PlayAnimation("idle")
...
local function enter_water_lobster(inst)
    local ix, iy, iz = inst.Transform:GetWorldPosition()
