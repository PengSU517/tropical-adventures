...
local function entrance()
	...
	if inst.entrada == nil then
		...
		inst.exit = SpawnPrefab("academy_door_saida")
		...
		----------------parede do fundo---------------------------------------------
		local part = SpawnPrefab("wallinterioracademy")
		if part ~= nil then
		...
		local part = SpawnPrefab("pigman_professor_shopkeep")
		if part ~= nil then
			part.Transform:SetPosition(x - 2.3, 0, z + 4)
		...
		if part ~= nil then
			part.Transform:SetPosition(x - 5, 0, z - 3.3)
			if part.components.health ~= nil then
				part.components.health:SetPercent(1)
			end
		end
		...
		local part = SpawnPrefab("deco_accademy_pig_king_painting")
		if part ~= nil then
			part.Transform:SetPosition(x - 5, 0, z + 3)
			part.Transform:SetRotation(180)
			if part.components.health ~= nil then
				part.components.health:SetPercent(1)
			end
		...
		if part ~= nil then
			part.Transform:SetPosition(x + 2, 0, z - 5.5)
			if part.components.health ~= nil then
				part.components.health:SetPercent(1)
			end
		end

		local part = SpawnPrefab("deco_accademy_vause")
		if part ~= nil then
		...
		local part = SpawnPrefab("deco_accademy_graniteblock")
		if part ~= nil then
			part.Transform:SetPosition(x - 2, 0, z - 6.5)
			if part.components.health ~= nil then
				part.components.health:SetPercent(1)
		...
		local part = SpawnPrefab("deco_accademy_table_books")
		if part ~= nil then
		...
		local part = SpawnPrefab("deco_accademy_potterywheel_urn")
		if part ~= nil then
		...
		if part ~= nil then
			part.Transform:SetPosition(x + 1, 0, z - 2.5)
			part.startAnim = "idle_stoneslab"
			part.AnimState:PlayAnimation("idle_stoneslab")
			part.shoptype = "pig_shop_academy"
			if part.components.health ~= nil then
				part.components.health:SetPercent(1)
			end
		end
		...
		------------------------portoes trancados--------------------------------
		local part = SpawnPrefab("pig_shop_academy_floor")
		if part ~= nil then
		...
		inst:DoTaskInTime(0, function(inst)
			local portaentrada = SpawnPrefab("pig_shop_academy")
...
local function wall_common(build)
	...
	inst.AnimState:PlayAnimation("shop_wall_circles", true)
...
return Prefab("pig_academy_entrance", entrance),
	Prefab("academy_door_saida", fnescada, assets),
	Prefab("pig_shop_academy_floor", SpawnPiso1, assets),
	Prefab("wallinterioracademy", wall_common, assets)
