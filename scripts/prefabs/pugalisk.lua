...
local function redirecthealth(inst, amount, overtime, cause, ignore_invincible, afflicter)
    ...
    elseif amount and inst.host then
        ...
        if inst.host.components.health then
            inst.host.components.health:DoDelta(amount, overtime, cause, ignore_invincible, nil, true) ---似乎修了又好像没修
        end
...
local function bodyfn(Sim)
    ...
    inst.components.groundpounder.groundpounddamagemult = 30 / PUGALISK_DAMAGE
