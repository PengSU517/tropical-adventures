...
local function entrance()
	...
	if inst.entrada == nil then
		...
		inst.exit = SpawnPrefab("pig_shop_arcane_door_saida")
		...
		----------------parede do fundo---------------------------------------------
		local part = SpawnPrefab("wallinteriorarcane")
		if part ~= nil then
		...
		local part = SpawnPrefab("pigman_erudite_shopkeep")
		if part ~= nil then
			part.Transform:SetPosition(x - 3, 0, z + 4)
		...
		local part = SpawnPrefab("shelves_glass")
		if part ~= nil then
		...
		local part = SpawnPrefab("deco_arcane_bookshelf")
		if part ~= nil then
			part.Transform:SetPosition(x - 4.5, 0, z)
			if part.components.health ~= nil then
				part.components.health:SetPercent(1)
			end
		end

		local part = SpawnPrefab("rug_round")
		if part ~= nil then
			part.Transform:SetPosition(x, 0, z)
			if part.components.health ~= nil then
				part.components.health:SetPercent(1)
		...
		local part = SpawnPrefab("containers")
		if part ~= nil then
			part.Transform:SetPosition(x + (15 / 2) - 3, 0, z - (15 / 2) + 1.5)
			if part.components.health ~= nil then
				part.components.health:SetPercent(1)
			end
		end

		local part = SpawnPrefab("deco_accademy_cornerbeam")
		if part ~= nil then
		...
		local part = SpawnPrefab("deco_accademy_beam")
		if part ~= nil then
		...
		local part = SpawnPrefab("deco_antiquities_screamcatcher")
		if part ~= nil then
		...
		local part = SpawnPrefab("deco_antiquities_windchime")
		if part ~= nil then
			part.Transform:SetPosition(x - 2, 0, z + 6.5)
			part.Transform:SetRotation(-90)
			if part.components.health ~= nil then
		...
		local part = SpawnPrefab("window_round_arcane")
		if part ~= nil then
			part.Transform:SetPosition(x, 0, z - 15 / 2)
			part.Transform:SetRotation(-90)
			if part.components.health ~= nil then
		...
		local part = SpawnPrefab("shop_buyer")
		if part ~= nil then
			part.Transform:SetPosition(x + 0.5, 0, z + (15 / 2) - 2.5)
			part.startAnim = "idle_marblesilk"
			part.AnimState:PlayAnimation("idle_marblesilk")
			part.shoptype = "pig_shop_arcane"
			if part.components.health ~= nil then
		...
		------------------------portoes trancados--------------------------------
		local part = SpawnPrefab("pig_shop_arcane_floor")
		if part ~= nil then
		...
		inst:DoTaskInTime(0, function(inst)
			local portaentrada = SpawnPrefab("pig_shop_arcane")
...
local function SpawnPiso1(inst)
	...
	inst.AnimState:PlayAnimation("shop_floor_octagon", true)
...
local function wall_common(build)
	...
	inst.AnimState:PlayAnimation("shop_wall_moroc", true)
...
return Prefab("pig_shop_arcane_entrance", entrance),
	Prefab("pig_shop_arcane_door_saida", fnescada, assets),
	Prefab("pig_shop_arcane_floor", SpawnPiso1, assets),
	Prefab("wallinteriorarcane", wall_common, assets)
