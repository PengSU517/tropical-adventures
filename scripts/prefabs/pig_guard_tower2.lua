...
local function citypossessionfn(inst)
    if inst:HasTag("palacetower") then
        ...
        local spawned = { "pig_royalguard_rich_2" }
        inst.components.spawner:Configure("pig_royalguard_rich_2", GUARDTOWER_CITY_RESPAWNTIME, 1)
    else
...
local function fn(Sim)
    ...
    inst.components.fixable:AddRecinstructionStageData("rubble", "pig_shop", "pig_guard_tower_palace2")
    inst.components.fixable:AddRecinstructionStageData("unbuilt", "pig_shop", "pig_guard_tower_palace2")

    inst:ListenForEvent("burntup", function(inst)
        inst.components.fixable:AddRecinstructionStageData("burnt", "pig_shop", "pig_guard_tower_palace2", 1)
        if inst.doortask then
            inst.doortask:Cancel()
            inst.doortask = nil
        end
...
return Prefab("common/objects/pig_guard_tower2", fn, assets, prefabs),
    Prefab("common/objects/pig_guard_tower_palace2", palacefn, assets, prefabs),
    MakePlacer("common/pig_guard_tower_placer2", "pig_shop", "pig_guard_tower_palace2", "idle"),
    MakePlacer("common/pig_guard_tower_palace_placer2", "pig_shop", "pig_tower_royal_build", "idle")
