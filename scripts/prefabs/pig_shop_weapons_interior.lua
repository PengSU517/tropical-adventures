...
local function entrance()
	...
	if inst.entrada == nil then
		...
		inst.exit = SpawnPrefab("pig_shop_weapons_door_saida")
		...
		----------------parede do fundo---------------------------------------------
		local part = SpawnPrefab("wallinteriorweapons")
		if part ~= nil then
		...
		local part = SpawnPrefab("window_square_weapons")
		if part ~= nil then
	...
	part.components.health:SetPercent(1)
		...
		------------------------portoes trancados--------------------------------
		local part = SpawnPrefab("pig_shop_weapons_floor")
		if part ~= nil then
...
local function wall_common(build)
	...
	inst.AnimState:PlayAnimation("shop_wall_upholstered", true)
...
local function fnescada()
	...
	inst.AnimState:PlayAnimation("idle_basic")
...
return Prefab("pig_shop_weapons_entrance", entrance),
	Prefab("pig_shop_weapons_door_saida", fnescada, assets),
	Prefab("pig_shop_weapons_floor", SpawnPiso1, assets),
	Prefab("wallinteriorweapons", wall_common, assets)
