...
local function entrance()
	...
	if inst.entrada == nil then
		...
		inst.exit = SpawnPrefab("pig_shop_hoofspa_door_saida")
		...
		----------------parede do fundo---------------------------------------------
		local part = SpawnPrefab("wallinteriorhoofspa")
		if part ~= nil then
		...
		local part = SpawnPrefab("pigman_beautician_shopkeep")
		if part ~= nil then
		...
		local part = SpawnPrefab("shelves_marble")
		if part ~= nil then
		...
		local part = SpawnPrefab("deco_marble_cornerbeam")
		if part ~= nil then
		...
		local part = SpawnPrefab("deco_marble_beam")
		if part ~= nil then
			part.Transform:SetPosition(x + 4.7, 0, z - 15 / 2 + 0.3)
			if part.components.health ~= nil then
				part.components.health:SetPercent(1)
			end
		end
		...
		local part = SpawnPrefab("deco_chaise")
		if part ~= nil then
			part.Transform:SetPosition(x - 1.4, 0, z - 3.5)
			if part.components.health ~= nil then
				part.components.health:SetPercent(1)
			end
		end

		local part = SpawnPrefab("deco_lamp_hoofspa")
		if part ~= nil then
			part.Transform:SetPosition(x - 1.9, 0, z - 5.2)
			if part.components.health ~= nil then
				part.components.health:SetPercent(1)
			end
		end

		local part = SpawnPrefab("deco_plantholder_marble")
		if part ~= nil then
			part.Transform:SetPosition(x - 4.6, 0, z + (15 / 2) - 2)
			if part.components.health ~= nil then
				part.components.health:SetPercent(1)
			end
		end

		local part = SpawnPrefab("deco_valence")
		if part ~= nil then
			part.Transform:SetPosition(x - 5.01, 0, z - 15 / 2)
			if part.components.health ~= nil then
				part.components.health:SetPercent(1)
			end
		end
		...
		local part = SpawnPrefab("swinging_light_floral_bulb")
		if part ~= nil then
		...
		if part ~= nil then
			part.Transform:SetPosition(x + 2.3, 0, z - 4.5)
			part.startAnim = "idle_cakestand"
			part.AnimState:PlayAnimation("idle_cakestand")
			part.shoptype = "pig_shop_hoofspa"
			if part.components.health ~= nil then
				part.components.health:SetPercent(1)
			end
		end
		...
		local part = SpawnPrefab("shop_buyer")
		if part ~= nil then
			part.Transform:SetPosition(x + 2.3, 0, z - 2.6)
			part.startAnim = "idle_cakestand"
			part.AnimState:PlayAnimation("idle_cakestand")
			part.shoptype = "pig_shop_hoofspa"
			if part.components.health ~= nil then
		...
		local part = SpawnPrefab("shop_buyer")
		if part ~= nil then
			part.Transform:SetPosition(x - 0.5, 0, z)
			part.startAnim = "idle_marble"
			part.AnimState:PlayAnimation("idle_marble")
			part.shoptype = "pig_shop_hoofspa"
			if part.components.health ~= nil then
				part.components.health:SetPercent(1)
			end
		...
		local part = SpawnPrefab("shop_buyer")
		if part ~= nil then
			part.Transform:SetPosition(x - 0.5, 0, z + 3)
			part.startAnim = "idle_marble"
			part.AnimState:PlayAnimation("idle_marble")
			part.shoptype = "pig_shop_hoofspa"
			if part.components.health ~= nil then
		...
		local part = SpawnPrefab("shop_buyer")
		if part ~= nil then
			part.Transform:SetPosition(x + 2, 0, z + 4.4)
			part.startAnim = "idle_marblesilk"
			part.AnimState:PlayAnimation("idle_marblesilk")
			part.shoptype = "pig_shop_hoofspa"
			if part.components.health ~= nil then
				part.components.health:SetPercent(1)
			end
		...
		------------------------portoes trancados--------------------------------
		local part = SpawnPrefab("pig_shop_hoofspa_floor")
		if part ~= nil then
		...
		inst:DoTaskInTime(0, function(inst)
			local portaentrada = SpawnPrefab("pig_shop_hoofspa")
...
local function wall_common(build)
	...
	inst.AnimState:PlayAnimation("shop_wall_marble", true)
...
return Prefab("pig_shop_hoofspa_entrance", entrance),
	Prefab("pig_shop_hoofspa_door_saida", fnescada, assets),
	Prefab("pig_shop_hoofspa_floor", SpawnPiso1, assets),
	Prefab("wallinteriorhoofspa", wall_common, assets)
