...
local function c()
    ...
    local f = Vector3()
    for g = 7, 0, -.25 do
        f.x, f.y, f.z = d.entity:LocalToWorldSpace(g, 0, 0)
        if e:IsPassableAtPoint(f:Get()) and not e:IsGroundTargetBlocked(f) then return f end
    end;
    return f
end;
local h = {
    fossil = function(i, j, f)
        if j.components.buffable then
            i.components.fossilizer:SetDuration(TUNING.FORGE_ITEM_PACK.TFWP_CONTROL_BOOK.DURATION *
            (j.components.buffable:GetBuffData("spell_duration_mult") + 1))
        else
            i.components.fossilizer:SetDuration(TUNING.FORGE_ITEM_PACK.TFWP_CONTROL_BOOK.DURATION)
        end;
        i.components.fossilizer:Fossilize(f, j)
        i.components.recarregavel:StartRecharge()
        i.components.aoespell:OnSpellCast(j)
    end,
    elemental = function(i, j, f)
        local k = SpawnPrefab("golem")
        k.Transform:SetPosition(f:Get())
        i.components.recarregavel:StartRecharge()
        if j.components.buffable then
            k.components.combat.damagemultiplier = j.components.buffable:GetBuffData("spell_dmg_mult") + 1; k.ischarged =
            k.components.combat.damagemultiplier > 1
        end;
        i.components.aoespell:OnSpellCast(j)
    end
}
...
local function m(n, o, p)
    ...
    local function s(i, t)
        ...
        t.AnimState:OverrideSymbol("book_closed", "swap_" .. q, "book_closed")
    end;
    ...
    local function u()
        ...
        i.entity:SetPristine()
        if not TheWorld.ismastersim then return i end;
        ...
        i.components.equippable:SetOnEquip(s)
        i.components.equippable:SetOnUnequip(onunequip)
        if n == "fossil" then
            i:AddComponent("fossilizer")
        end;
        ...
        i.components.weapon:SetDamage(TUNING.FORGE_ITEM_PACK
        [string.upper(n == "fossil" and "tfwp_control_book" or "tfwp_summon_book")].DAMAGE)
        i.components.weapon:SetDamageType(DAMAGETYPES.PHYSICAL)
        return i
    end;
    return
        CustomPrefab(n == "fossil" and "tfwp_control_book" or "tfwp_summon_book", u, r, p, nil,
            "images/inventoryimages.xml", q .. ".tex",
            TUNING.FORGE_ITEM_PACK[string.upper(n == "fossil" and "tfwp_control_book" or "tfwp_summon_book")])
end;
