...
local function OnGetItemFromPlayer(inst, giver, item)
    ...
    if item.components.edible ~= nil then
        --meat makes us friends (unless I'm a guard)  例如猪皮的食物类型为HORRIBLE
        if item.components.edible.foodtype == FOODTYPE.MEAT or item.components.edible.foodtype == FOODTYPE.HORRIBLE then
            ...
            elseif giver and giver.components.leader ~= nil and not inst:HasTag("guard") then
...
local function NormalRetargetFn(inst)
    return FindEntity(
        ...
        { "monster", "_combat" },                  -- see entityreplica.lua  --必须有的tag
...
local function common(moonbeast)
    ...
    MakeCharacterPhysics(inst, 500, 0.75)
