...
local function entrance()
	...
	if inst.entrada == nil then
		...
		inst.exit = SpawnPrefab("pig_shop_tinker_door_saida")
		...
		----------------parede do fundo---------------------------------------------
		local part = SpawnPrefab("wallinteriortinker")
		if part ~= nil then
		...
		local part = SpawnPrefab("pigman_mechanic_shopkeep")
		if part ~= nil then
		...
		local part = SpawnPrefab("deco_tinker_cornerbeam")
		if part ~= nil then
		...
		local part = SpawnPrefab("deco_tinker_beam")
		if part ~= nil then
		...
		if part ~= nil then
			part.Transform:SetPosition(x - depth / 4, 0, z + 15 / 2)
		...
		local part = SpawnPrefab("shelves_metal")
		if part ~= nil then
		...
		if part ~= nil then
			...
			part.Transform:SetRotation(100)
			if part.components.health ~= nil then
				part.components.health:SetPercent(1)
			end
		end
		...
		if part ~= nil then
			part.Transform:SetPosition(x - 3, 0, z - width / 4.5 + 0.5)
			if part.components.health ~= nil then
				part.components.health:SetPercent(1)
			end
		end
		...
		local part = SpawnPrefab("swinging_light_bank")
		if part ~= nil then
			part.Transform:SetPosition(x, 0, z + width / 4.5 + 2)
			if part.components.health ~= nil then
				part.components.health:SetPercent(1)
			end
		end

		local part = SpawnPrefab("deco_rollchest")
		if part ~= nil then
			part.Transform:SetPosition(x + 4, 0, z - 5)
			if part.components.health ~= nil then
				part.components.health:SetPercent(1)
			end
		end

		local part = SpawnPrefab("deco_worktable")
		if part ~= nil then
			part.Transform:SetPosition(x + 2.5, 0, z + 4)
			part.Transform:SetRotation(90)
			if part.components.health ~= nil then
				part.components.health:SetPercent(1)
			end
		...
		local part = SpawnPrefab("deco_filecabinet")
		if part ~= nil then
			part.Transform:SetPosition(x - 2.5, 0, z - width / 2)
			if part.components.health ~= nil then
				part.components.health:SetPercent(1)
			end
		end

		local part = SpawnPrefab("deco_rollholder")
		if part ~= nil then
			part.Transform:SetPosition(x + 2, 0, z - width / 2 + 0.7)
			if part.components.health ~= nil then
				part.components.health:SetPercent(1)
			end
		end
		...
		local part = SpawnPrefab("deco_rollholder")
		if part ~= nil then
			part.Transform:SetPosition(x, 0, z + width / 2 - 0.7)
			part.Transform:SetRotation(90)
			if part.components.health ~= nil then
				part.components.health:SetPercent(1)
			end
		...
		local part = SpawnPrefab("deco_rollholder_front")
		if part ~= nil then
			part.Transform:SetPosition(x - depth / 2 + 0.3, 0, z - 4)
			part.Transform:SetRotation(90)
			if part.components.health ~= nil then
				part.components.health:SetPercent(1)
			end
		...
		if part ~= nil then
			part.Transform:SetPosition(x + 1.3, 0, z - width / 6 - 1.75)
			part.startAnim = "idle_metal"
			part.AnimState:PlayAnimation("idle_metal")
			part.shoptype = "pig_shop_tinker"
			if part.components.health ~= nil then
				part.components.health:SetPercent(1)
			end
		end
		...
		local part = SpawnPrefab("shop_buyer")
		if part ~= nil then
			part.Transform:SetPosition(x + 1.3, 0, z - 1.75)
			part.startAnim = "idle_metal"
			part.AnimState:PlayAnimation("idle_metal")
			part.shoptype = "pig_shop_tinker"
			if part.components.health ~= nil then
		...
		local part = SpawnPrefab("shop_buyer")
		if part ~= nil then
			part.Transform:SetPosition(x - 1.7, 0, z + width / 6 + 0.5)
			part.startAnim = "idle_metal"
			part.AnimState:PlayAnimation("idle_metal")
			part.shoptype = "pig_shop_tinker"
			if part.components.health ~= nil then
		...
		local part = SpawnPrefab("shop_buyer")
		if part ~= nil then
			part.Transform:SetPosition(x - 1.7, 0, z + 0.5)
			part.startAnim = "idle_metal"
			part.AnimState:PlayAnimation("idle_metal")
			part.shoptype = "pig_shop_tinker"
			if part.components.health ~= nil then
		...
		-----------------------portoes trancados--------------------------------
		local part = SpawnPrefab("pig_shop_tinker_floor")
		if part ~= nil then
		...
		inst:DoTaskInTime(0, function(inst)
			local portaentrada = SpawnPrefab("pig_shop_tinker")
...
local function wall_common(build)
	...
	inst.AnimState:PlayAnimation("shop_wall_bricks", true)
...
return Prefab("pig_shop_tinker_entrance", entrance),
	Prefab("pig_shop_tinker_door_saida", fnescada, assets),
	Prefab("pig_shop_tinker_floor", SpawnPiso1, assets),
	Prefab("wallinteriortinker", wall_common, assets)
