...
local function entrance()
	...
	if inst.entrada == nil then
		...
		----------------parede do fundo---------------------------------------------
		local part = SpawnPrefab("wallinteriorcityhall")
		if part ~= nil then
		...
		local part = SpawnPrefab("pigman_mayor_shopkeep")
		if part ~= nil then
		...
		local part = SpawnPrefab("swinging_light_basic_bulb")
		if part ~= nil then
			part.Transform:SetPosition(x, 0, z)
			if part.components.health ~= nil then
				part.components.health:SetPercent(1)
		...
		if part ~= nil then
			part.Transform:SetPosition(x - 10 / 2, 0, z + 15 / 2 + 0.6)
			part.Transform:SetRotation(90)
			if part.components.health ~= nil then
				part.components.health:SetPercent(1)
			end
		...
		if part ~= nil then
			part.Transform:SetPosition(x, 0, z + 15 / 2 - 3)
			part.startAnim = "idle_globe_bar"
			part.AnimState:PlayAnimation("idle_globe_bar")
			part.saleitem = { "deed", "oinc", 50 }
			if part.components.health ~= nil then
				part.components.health:SetPercent(1)
			end
		end
		...
		local part = SpawnPrefab("shop_buyer")
		if part ~= nil then
			part.Transform:SetPosition(x + 2, 0, z - 15 / 2 + 3)
			part.startAnim = "idle_marble_dome"
			part.AnimState:PlayAnimation("idle_marble_dome")
			part.saleitem = { "securitycontract", "oinc", 10 }
			if part.components.health ~= nil then
				part.components.health:SetPercent(1)
			end
		end
		...
		------------------------portoes trancados--------------------------------
		local part = SpawnPrefab("pig_shop_cityhall_floor")
		if part ~= nil then
...
return Prefab("pig_shop_cityhall_entrance", entrance),
	Prefab("pig_shop_cityhall_door_saida", fnescada, assets),
	Prefab("pig_shop_cityhall_floor", SpawnPiso1, assets),
	Prefab("wallinteriorcityhall", wall_common, assets)
