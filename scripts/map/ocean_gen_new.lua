...
local function is_tropical_oceanlined(tile)
	return SW_LAND_TILES[tile] or
		(tile == WORLD_TILES.MANGROVE)
end
...
local function IsCloseToWater(x, y, radius)
	for i = -radius, radius, 1 do
		if IsWaterTile(world:GetTile(x - radius, y + i)) or IsWaterTile(world:GetTile(x + radius, y + i)) then
			return true
		end
	end
	for i = -(radius - 1), radius - 1, 1 do
		if IsWaterTile(world:GetTile(x + i, y - radius)) or IsWaterTile(world:GetTile(x + i, y + radius)) then
			return true
		end
	end
	return false
...
local function do_groundfill(fillTile, fillOffset, fillDepth, landRadius, width, height, data)
	print("[Ocean]  Ground fill...")
	for y = 0, height - 1, 1 do
		for x = 0, width - 1, 1 do
			local ground = world:GetTile(x, y)
			if is_oceanlined(ground) then
...
local function AddShoreline(width, height, data)
	...
	for y = 0, height - 1, 1 do
		for x = 0, width - 1, 1 do
			local ground = world:GetTile(x, y)
			if IsWaterTile(ground) and not IsSurroundedByWaterOrInvalid(x, y, 1) then
				if ground ~= WORLD_TILES.OCEAN_BRINEPOOL then
					world:SetTile(x, y, WORLD_TILES.OCEAN_COASTAL_SHORE) --WORLD_TILES.OCEAN_COASTAL_SHORE
				end
			end
		end
	end
		...
		while i < adj_width and (count == nil or #positions < count) do
			...
			if checkTile(x2 + min_dist_from_land, y - min_dist_from_land, populating_tile) and checkTile(x2 + min_dist_from_land, y2 + min_dist_from_land, populating_tile) then
				if checkTile(x - min_dist_from_land, y - min_dist_from_land, populating_tile) and checkTile(x - min_dist_from_land, y2 + min_dist_from_land, populating_tile) then
					...
					--check all tiles
					local ok, last_x, last_y = checkAllTiles(populating_tile, x - min_dist_from_land,
						y - min_dist_from_land, x2 + min_dist_from_land, y2 + min_dist_from_land)
					if ok == true then
						--bottom-left
...
local function PlaceOceanLayout(layout, prefabs, populating_tile, ReserveAndPlaceLayoutFn, min_dist_from_land)
	...
	local positions = findLayoutPositions(layoutsize, OCEAN_WATERFALL_MAX_DIST + 2, populating_tile, 1,
		min_dist_from_land)
	if #positions > 0 then
...
function Ocean_PlaceSetPieces(set_pieces, add_entity, obj_layout, populating_tile, min_dist_from_land, encoded_topology,
	...
	if set_pieces ~= nil then
		...
		local function ReserveAndPlaceLayoutFn(layout, prefabs, position, area_size)
			...
			if layout.add_topology ~= nil then
				AddSquareTopology(encoded_topology, position[1], position[2], area_size, map_width, map_height,
					layout.add_topology)
			end
		end
...
function PopulateWaterPrefabWorldGenCustomizations(populating_tile, spawnFn, entitiesOut, width, height, edge_dist,
												   water_contents, world_gen_choices, prefab_list)
	...
	for idx = 1, math.min(#points_x, pos_needed) do
		...
		if prefab ~= nil then
			...
			PopulateWorld_AddEntity(prefab, points_x[idx], points_y[idx], populating_tile, entitiesOut, width, height,
...
local function PopulateWaterType(populating_tile, spawnFn, entitiesOut, width, height, edge_dist, water_contents,
								 world_gen_choices, min_dist_from_land, encoded_topology)
	...
	if water_contents.countstaticlayouts ~= nil and next(water_contents.countstaticlayouts) ~= nil then
		local add_fn = {
			fn = function(prefab, points_x, points_y, idx, entitiesOut, width, height, prefab_list, prefab_data,
						  rand_offset)
				PopulateWorld_AddEntity(prefab, points_x[idx], points_y[idx], populating_tile, entitiesOut, width, height,
					prefab_list, prefab_data, rand_offset)
			end,
			args = { entitiesOut = entitiesOut, width = width, height = height, rand_offset = true, debug_prefab_list = setpiece_prefab_list }
		}
		Ocean_PlaceSetPieces(water_contents.countstaticlayouts, add_fn, obj_layout, populating_tile, min_dist_from_land,
			encoded_topology, width, height)
	end
	...
	if water_contents.countprefabs ~= nil then
		...
		-- 			local prefab_data = {}
		-- 			prefab_data.data = water_contents.prefabdata and water_contents.prefabdata[prefab] or nil
		-- 			PopulateWorld_AddEntity(prefab, points_x[pos_cur], points_y[pos_cur], populating_tile, entitiesOut,
		-- 				width, height, prefab_list, prefab_data)
		-- 			added = added + 1
		-- 		end
				...
				if world:GetTile(x, y) == populating_tile then
					if math.random() < water_contents.distributepercent then
						...
						if prefab ~= nil then
							local prefab_data = {}
							prefab_data.data = water_contents.prefabdata and water_contents.prefabdata[prefab] or nil
							PopulateWorld_AddEntity(prefab, x, y, populating_tile, entitiesOut, width, height,
								prefab_list, prefab_data)
						end
					end
				end
	...
	PopulateWaterPrefabWorldGenCustomizations(populating_tile, spawnFn, entitiesOut, width, height, edge_dist,
		water_contents, world_gen_choices, prefab_list)
...
function PopulateOcean(spawnFn, entitiesOut, width, height, ocean_contents, world_gen_choices, min_dist_from_land,
					   encoded_topology)
	...
	if ocean_contents ~= nil then
		for i, room in ipairs(ocean_contents) do
			PopulateWaterType(room.data.value, spawnFn, entitiesOut, width, height, OCEAN_POPULATION_EDGE_DIST,
				room.data.contents, world_gen_choices, min_dist_from_land, encoded_topology)
		end
	end
end
