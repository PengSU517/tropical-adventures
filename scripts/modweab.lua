...
local k = _G.TheNet; AddPrefabPostInit("inventoryitem_classified",
    function(l)
        l.isaltattacking = h(l.GUID, "weapon.isaltattacking")
        l.altattackrange = i(l.GUID, "weapon.altattackrange")
    end)
AddPrefabPostInit("player_classified",
    function(l)
        l.net_buffs = j(l.GUID, "forge.net_buffs", "net_buffs_dirty")
        if not k:IsDedicated() then
            l:ListenForEvent("net_buffs_dirty",
                function(l)
                    if not l._parent or not l._parent.HUD or #l.net_buffs:value() < 1 then return end
...
AddPlayerPostInit(function(l)
    l:AddComponent("buffable")
    if not l.components.colourfader then l:AddComponent("colourfader") end
end)
...
_G.DAMAGETYPE_IDS = {}
for o, p in pairs(_G.DAMAGETYPES) do _G.DAMAGETYPE_IDS[p] = o end
;
AddComponentPostInit("combat", function(self)
    ...
    function self:AddDamageBuff(r, s, v)
        if not self:HasDamageBuff(r, s) then
            self.damagebuffs[r and "recieved" or "dealt"].generic[s] =
                v
        end
    end
    ...
    function self:HasDamageBuff(r, s, t)
        local u = self.damagebuffs[r and "recieved" or "dealt"]
            [type(t) == "number" and "dmgtype" or (type(t) == "string" and "stimuli" or "generic")]
        if not t then return u[s] ~= nil else return u[t] ~= nil and u[t][s] ~= nil end
    end
    ...
    local function F(r, G, H, z, x)
        ...
        ;
        for S, v in pairs(K[R and "overrides" or "common"].multiplicative) do if v then J = J * v end end
        ; for S, v in pairs(K[R and "overrides" or "common"].additive) do if v then I = I * (v + 1) end end
        ;
        if I == 0 then I = 1 end
        ;
        if J == 0 then J = 1 end
    ...
    function self:GetBuffMults(r, G, H, z, x) return F(r, G, H, z, x) end
...
AddComponentPostInit("weapon",
    function(self)
        self.hasaltattack = false; self.isaltattacking = false; self.altattackrange = nil; self.althitrange = nil; self.altdamage = nil; self.altdamagecalc = nil; self.altcondition = nil; self.altprojectile = nil; self._projectile = nil; self.damagetype = nil; self.altdamagetype = nil; self._damagetype = nil;
        ...
        function self:SetAltAttack(X, Y, a6, q, a7, a8)
            ...
            ; self.altprojectile = a6; self.altdamagetype = q ~= nil and q or self.damagetype; self.altdamagecalc = a7; self.altcondition =
        ...
        function self:SetIsAltAttacking(aa, ab)
            self.isaltattacking = aa; if self.inst.replica.inventoryitem then
                self.inst.replica.inventoryitem
                    :SetIsAltAttacking(aa)
            end
            ; if aa then
...
AddComponentPostInit("health", function(self)
    ...
    function self:GetHealthBuff()
        local ad = 0; for s, v in pairs(self.buffs) do ad = ad + v end
        ; return ad
    end
    ...
    function self:AddHealthBuff(s, v)
        if not self:HasHealthBuff(s) then
            self.buffs[s] = v; self.maxhealth = self.maxhealth + self:GetHealthBuff()
...
AddComponentPostInit("equippable",
    function(self)
        ...
        function self:AddHealthBuff(s, v)
            if not self:HasHealthBuff(s) then
                local ae = self.inst.components.inventoryitem.owner; if ae.components.health then
                    ae.components.health
                        :AddHealthBuff(s, v)
                end
            end
        end
        ...
        ; function self:RemoveHealthBuff(s)
            if self:HasHealthBuff(s) then
                self.healthbuffs[s] = nil; local ae = self.inst.components.inventoryitem.owner; if ae.components.health then
        ...
        function self:AddDamageBuff(r, s, v)
            if not self:HasDamageBuff(r, s) then
                self.damagebuffs[r and "recieved" or "dealt"].generic[s] = v; if self:IsEquipped() then
                    local ae = self.inst.components.inventoryitem.owner; if ae.components.combat then
                        ae.components
                            .combat:AddDamageBuff(r, s, v)
                    end
                end
            end
        end
        ...
        function self:AddDamageTypeBuff(r, w, s, v)
            if not self:HasDamageBuff(r, s, w) then
                local u = self.damagebuffs[r and "recieved" or "dealt"].dmgtype; if not u[w] then u[w] = {} end
                ; u[w][s] = v; if self:IsEquipped() then
                    local ae = self.inst.components.inventoryitem.owner; if ae.components.combat then
                        ae.components
                            .combat:AddDamageTypeBuff(r, w, s, v)
                    end
                end
            end
        end
        ...
        ; function self:AddDamageStimuliBuff(r, x, s, v)
            if not self:HasDamageBuff(r, s, x) then
                local u = self.damagebuffs[r and "recieved" or "dealt"].stimuli; if not u[x] then u[x] = {} end
                ; u[x][s] = v; if self:IsEquipped() then
                    local ae = self.inst.components.inventoryitem.owner; if ae.components.combat then
                        ae.components
                            .combat:AddDamageStimuliBuff(r, x, s, v)
                    end
                end
            end
        end
        ...
        function self:RemoveDamageBuff(r, s, t)
            if self:HasDamageBuff(r, s, t) then
                ...
                if not t then
                    ...
                    u[t][s] = nil; if _G.GetTableSize(u[t]) < 1 then u[t] = nil end
                end
                ; if self:IsEquipped() then
                    local ae = self.inst.components.inventoryitem.owner; if ae.components.combat then
                        ae.components
                            .combat:RemoveDamageBuff(r, s, t)
                    end
                end
            end
        end
        ...
        function self:AddUniqueBuff(af)
            for t, v in pairs(af) do self.uniquebuffs[t] = v end
            ; if self:IsEquipped() then
        ...
        function self:RemoveUniqueBuff(t)
            self.uniquebuffs[t] = nil; if self:IsEquipped() then
                local ae = self.inst.components.inventoryitem.owner; self:AddBuffsTo(ae)
            end
        end
                            ...
                            if o == "generic" then
                                ...
                                for ah, ai in pairs(v) do
                                    if o == "dmgtype" then
                                        ...
                                        ae
                                            .components.combat:AddDamageStimuliBuff(P == "recieved", t, ah, ai)
                                    end
                                end
                            end
                        end
                        ...
                        for t, v in pairs(K) do
                            if o == "generic" then
                                ...
                                for s, S in pairs(v) do
                                    ae
                                        .components.combat:RemoveDamageBuff(P == "recieved", s, t)
                                end
                            end
                        end
                    end
...
AddClassPostConstruct("components/inventoryitem_replica",
    function(self)
        self.can_altattack_fn = function() return true end; function self:SetIsAltAttacking(aa)
            self.classified
                .isaltattacking:set(aa)
        end

        ; function self:SetAltAttackRange(Y) self.classified.altattackrange:set(Y or self:AttackRange()) end

        ; function self:AltAttackRange()
            if self.inst.components.weapon then
                return self.inst.components.weapon
                    .altattackrange
        ...
        ;
        function self:SetAltAttackCheck(ak) if ak and type(ak) == "function" then self.can_altattack_fn = ak end end

        ; function self:CanAltAttack()
            if self.classified.isaltattacking:value() then
                return self.inst.components.weapon and
                    self.components.weapon:CanAltAttack() or self.can_altattack_fn(self.inst)
            end
        end

        ; _oldAttackRange = self.AttackRange; function self:AttackRange()
            return self:CanAltAttack() and
                self:AltAttackRange() or _oldAttackRange(self)
        end
    end)
AddClassPostConstruct("components/combat_replica",
    function(self)
        local al = self.CanBeAttacked; function self:CanBeAttacked(G)
            if 1 == 2 and
                (self.inst:HasTag("companion") and G and G:HasTag("player")) then
                return false
            end
            ;
...
AddStategraphPostInit("wilson",
    function(self)
        local ap = self.states.attack.onenter; self.states.attack.onenter = function(l)
            local an = l.components.inventory:GetEquippedItem(c.HANDS)
            if an then am(l, an) end
            ; ap(l)
        end;
        ...
        self.actionhandlers[g.CASTAOE].deststate = function(l, ar)
            if ar.invobject:HasTag("parry") then return "parry_pre" end
            ; return aq(l, ar)
        end
    end)

AddStategraphPostInit("wilson_client",
    function(self)
        local ap = self.states.attack.onenter; self.states.attack.onenter = function(l)
        ...
        local aq = self.actionhandlers[g.CASTAOE].deststate; self.actionhandlers[g.CASTAOE].deststate = function(l, ar)
            if ar.invobject:HasTag("parry") then return "parry_pre" end
            ; return aq(l, ar)
        end
    end)

AddComponentPostInit("debuffable",
    function(self)
        local as = self.AddDebuff; local at = self.RemoveDebuff; function self:AddDebuff(m, ...)
            if self.debuffs[m] == nil and self.inst.player_classified then
                self.inst.player_classified.net_buffs:set(a
                    .encode({ name = m, removed = false }))
            end
            ; as(self, m, ...)
        end
        ...
        function self:RemoveDebuff(m, ...)
            if self.debuffs[m] ~= nil and self.inst.player_classified then
                self.inst.player_classified.net_buffs:set(a
                    .encode({ name = m, removed = true }))
            end
            ; at(self, m, ...)
        end
    end)
local au = { book = 25 * b, castspell = 53 * b }
local ac = function(...) end; for av, aw in pairs(au) do
    AddStategraphPostInit("wilson",
        function(self)
            local ax, ay, az = self.states[av].onenter or ac, self.states[av].onexit or ac,
                ...
                l.sg.statemem.tm = 0; l.sg.statemem.trgt = aw; l.sg.statemem.equip = l.components.inventory
                    :GetEquippedItem(c.HANDS)
                ax(l)
                ...
                ; if l.sg.statemem.tm < l.sg.statemem.trgt and l.sg.statemem.equip.components.reticule_spawner then
                    l.sg
                        .statemem.equip.components.reticule_spawner:Interrupt()
                end
                ; ay(l)
            end; self.states[av].onupdate = function(l, aA)
                l.sg.statemem.tm = l.sg.statemem.tm + aA; az(l)
            end
        end)
end
; local aB = { "spider", "spider_warrior", "tentacle", "hound", "firehound", "icehound", "krampus", "knight", "bishop",
    "rook", "knight_nightmare", "bishop_nightmare", "rook_nightmare", "spider_hider", "spider_spitter", "spider_dropper",
    "bat", "worm", "spat", "eyeplant", "walrus", "little_walrus", "monkey", "slurper", "tallbird", "frog", "mosquito",
    "bee", "killerbee", "pigguard", "moonpig", "merm", "leif", "leif_sparse", "spiderqueen", "deerclops", "minotaur",
    "warg", "birchnutdrake", "deciduoustree", "bearger", "moose", "dragonfly", "lavae", "beeguard", "cookiecutter",
    "mutatedhound", "houndcorpse", "mutated_penguin", "spider_moon", "beequeen", "klaus", "stalker", "stalker_forest",
    "stalker_atrium", "malbatross", "pigman", "koalefant_summer", "koalefant_winter", "slurtle", "snurtle", "beefalo",
    "penguin", "bunnyman", "rocky", "lightninggoat", "catcoon", "buzzard", "mossling", "antlion", "gestalt", "gnarwail",
    "gnarwail_attack_horn", "fruitdragon", "squid", "player" }
for S, aC in ipairs(aB) do
    AddPrefabPostInit(aC,
        function(l) if l.components and l.components.combat then l.components.combat:SetDamageType(1) end end)
end
