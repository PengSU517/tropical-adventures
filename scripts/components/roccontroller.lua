...
function RocController:OnUpdate(dt)
	...
	if tile == GROUND.OCEAN_BRINEPOOL or
		...
		tile == GROUND.INVALID or
		tile == GROUND.IMPASSABLE then --  tile == GROUND.FIELDS or
		onvalidtiles = false
	end

	local onvaliddungtiles = true
	if tile == GROUND.RAINFOREST or tile == GROUND.PLAINS or tile == GROUND.DEEPRAINFOREST then
					...
					if headdistsq > HEAD_ATTACK_DIST * HEAD_ATTACK_DIST then
						...
						if self.target:HasTag("isinventoryitem") or self.target:HasTag("player") then
							...
							if headdistsq < 0.2 then
								...
								-- self.head:PushEvent("exit")
								-- self.leg1:PushEvent("exit")
								-- self.leg2:PushEvent("exit")
								-- self.tail:PushEvent("exit")
							end
						else
