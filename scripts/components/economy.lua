local TRADER = {
	...
	pigman_queen = {
		...
		reward = "key_to_city", --  "pedestal_key",
		rewardqty = 1
	},
	pigman_usher = {
		items = { "honey", "jammypreserves", "icecream", "pumpkincookie", "waffles", "berries", "berries_cooked", "berries_juicy", "berries_juicy _cooked" },
		delay = 0,
		reset = 1,
		current = 0,
...
function Economy:GetDelay(traderprefab, city, inst)
	...
	elseif traderprefab == "pigman_collector" then
		return TRADER.pigman_collector.delay
	else
		return 0
	end
end
...
function Economy:MakeTrade(traderprefab, city, inst)
	...
	elseif traderprefab == "pigman_collector" then
		return TRADER.pigman_collector.reward, TRADER.pigman_collector.rewardqty
	else
		return "oinc", 1
	end
end
