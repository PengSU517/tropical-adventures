...
return Class(function(self, inst)
	...
	local GNARWAIL_SPAWN_CHANCE = 0.2
	...
	function self:ShouldSpawnANewSchoolForPlayer(player, percent_ocean)
		...
		if percent_ocean > 0.1 then
			local num_school_spawn_blockers = #TheSim:FindEntities(pt.x, pt.y, pt.z, SCHOOL_SPAWNER_FISH_CHECK_RADIUS,
				{ "tropicalspawnblocker" })
			if math.random() < 1 - num_school_spawn_blockers * SCHOOL_SPAWNER_BLOCKER_MOD then
				local pt = player:GetPosition()
				local num_fish = #TheSim:FindEntities(pt.x, pt.y, pt.z, SCHOOL_SPAWNER_FISH_CHECK_RADIUS,
					{ "tropicalspawner" })
				local r = math.random()
