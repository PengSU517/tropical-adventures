...
BIRD_TYPES[GROUND.RAINFOREST] = {"toucan_hamlet","kingfisher","parrot_blue", "kingfisher_swarm", "toucan_hamlet_swarm", "parrot_blue_swarm"}
BIRD_TYPES[GROUND.DEEPRAINFOREST]	= {"toucan_hamlet","parrot_blue","kingfisher", "kingfisher_swarm", "toucan_hamlet_swarm", "parrot_blue_swarm"}
BIRD_TYPES[GROUND.GASJUNGLE]	= {"parrot_blue", "parrot_blue_swarm"}
BIRD_TYPES[GROUND.FOUNDATION] = {"quagmire_pigeon","crow","pigeon_swarm"}
...
local function PickBird(spawnpoint)
	...
	if TheNet:GetServerGameMode() == "quagmire" then
		...
		if bird == "parrot" and math.random() < 0.2 then
				bird = "parrot_pirate"
		end
...
function self:SpawnBird(spawnpoint, ignorebait)
    ...
    if prefab == nil or prefab == "log" then
        return
    end
