...
local MAPWRAPPER_TELEPORT_RANGE = 8
local MAPWRAPPER_WARN_RANGE = 20
local MAPWRAPPER_GAINCONTROL_RANGE = 14
local MAPWRAPPER_LOSECONTROL_RANGE = 14
--local MAPWRAPPER_EDGEFOG_RANGE = 6

local MapWrapper = Class(function(self, inst)
	self.inst = inst
	self.state = STATE_WAIT
	self.edgefog = SpawnPrefab("edgefog")
	self.edgefog:AddTarget(inst)
	if TheWorld.ismastersim then
		self.inst:StartUpdatingComponent(self)
	end
end)

function MapWrapper:OnRemoveEntity()
	if self.edgefog then self.edgefog:Remove() end
end
...
local function get_dest(inst, x1, y1, z1, rads, prints)
	...
	local dist = (MAPWRAPPER_TELEPORT_RANGE + 2 * MAPWRAPPER_WARN_RANGE) * TILE_SCALE
...
local function get_angle2(x, y, z, rads)
	...
	local dirs = {}
	local len = 2 * MAPWRAPPER_WARN_RANGE * TILE_SCALE
	...
	for i = 1, #dirs, 1 do
		for j = 1, #edges, 1 do
			local newd = line_intersect_dist(x, z, x + dirs[i].x, z + dirs[i].z, edges[j].x3, edges[j].z3, edges[j].x4,
				edges[j].z4)
...
function MapWrapper:OnUpdate(dt)
	...
	elseif self.state == STATE_MOVETOEDGE then
		...
		self.inst.Physics:SetMotorVelOverride(TUNING.WILSON_RUN_SPEED, 0, 0)
		if is_inrange(MAPWRAPPER_TELEPORT_RANGE) then
