...
function AreaAwareness:GetDebugString()
	local s = string.format("%s: %s [%d] depth: %s", tostring(self.current_area.story), tostring(self.current_area.type),
		self.current_area.idx, tostring(self.current_area.story_depth) or "nil")
	return s
end

function AreaAwareness:StartCheckingPosition(checkinterval)
	self.checkpositiontask = self.inst:DoPeriodicTask(checkinterval or self.checkinterval,
		function() self:UpdatePosition() end)
end
