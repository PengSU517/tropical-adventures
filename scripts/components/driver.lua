...
self.vehicle.Transform:SetRotation(self.inst.Transform:GetRotation())
	...
	if gastabarco then
		if gastabarco.components.container and not gastabarco.components.container:IsOpen() and self.inst.sg:HasStateTag("sailing") then
			gastabarco.components.container:Open(self.inst)
		end
		...
		if self.vehicle.components.armor and gastabarco.components.armor then
			self.vehicle.components.armor.condition =
				gastabarco.components.armor.condition
		end
		if gastabarco.components.armor and gastabarco.components.armor.condition > self.vehicle.components.finiteuses.current then
			gastabarco.components.armor.condition =
				self.vehicle.components.finiteuses.current
		end
		if gastabarco.components.armor and gastabarco.components.armor.condition < self.vehicle.components.finiteuses.current then
			self.vehicle.components.finiteuses.current =
				gastabarco.components.armor.condition
	...
	if gastabarco and gastabarco.components.container then
		...
		if sailslot ~= nil and self.vehicle ~= nil and sailslot.prefab == "trawlnet" then
			...
			if ground and self.inst.components.locomotor.isrunning and
				TileGroupManager:IsOceanTile(ground) --[[(ground == GROUND.OCEAN_WATERLOG or
			...
			if ground and ground == GROUND.OCEAN_ROUGH and self.inst.components.locomotor.isrunning then
				sailslot.medio =
					sailslot.medio + 0.003
			end
			if ground and ground == GROUND.OCEAN_HAZARDOUS and self.inst.components.locomotor.isrunning then
				sailslot.fundo =
					sailslot.fundo + 0.003
			end
...
function Driver:OnMount(vehicle)
	...
	if self.inst.components.inventory:GetEquippedItem(EQUIPSLOTS.BARCO) and
		self.inst.components.inventory:GetEquippedItem(EQUIPSLOTS.BARCO) == vehicle.prefab then
		return
	end
