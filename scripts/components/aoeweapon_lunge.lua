...
function a:SetSideRange(range)
    self.siderange = range
end

function a:SetOnPreLungeFn(fn)
    self.onprelungefn = fn
end

function a:SetOnLungedFn(fn)
    self.onlungedfn = fn
end

function a:SetTrailFX(prefab, spacing)
...
function a:SetWidth(c) self.width = c end;

function a:SetStimuli(d)
    self.stimuli = d
end;

function a:SetOnLungeFn(e)
    self.onlunge = e
end;

function a:DoLunge(doer, startingpos, targetpos)
    local namao = doer.components.inventory:GetEquippedItem(EQUIPSLOTS.HANDS)
    if namao and (namao.prefab == "tfwp_spear_gung") then
        local i = {}
        local j = {}
        for k = 0, 10 do
            doer:DoTaskInTime(FRAMES * math.ceil(1 + k / 3.5),
                function()
                    local l = (targetpos - startingpos):GetNormalized() * k * 0.6; SpawnPrefab("spear_gungnir_lungefx")
                        .Transform:SetPosition((startingpos + l):Get())

                    local m, n, o = (startingpos + l):Get()
                    local p = TheSim:FindEntities(m, n, o, self.width / 2, nil, { "player", "companion" })
                    for q, r in ipairs(p) do
                        if not i[r] and r ~= doer and doer.components.combat:IsValidTarget(r) and (r.components.health or r.entity:IsValid() and r.components.workable and r.components.workable:CanBeWorked() and r.components.workable:GetWorkAction()) then
                            i[r] = true;
                            table.insert(j, r)
                        end
                    end;
                    if k == 10 then
                        if self.inst.components.weapon and self.inst.components.weapon:HasAltAttack() then
                            self.inst.components.weapon:DoAltAttack(doer, j, nil, self.stimuli)
                        end
                    end
                end)
        end;
        if self.onlunge ~= nil then self.onlunge(self.inst, doer, startingpos, targetpos) end;
        return true
    else
        ...
        local p2_hit_targets = TheSim:FindEntities(p2.x, 0, p2.y, doer_combat.hitrange + self.physicspadding, nil,
        ...
        local p2_toss_targets = TheSim:FindEntities(p2.x, 0, p2.y, doer_combat.hitrange + self.physicspadding,
            TOSS_MUSTTAGS, TOSS_CANTTAGS)
