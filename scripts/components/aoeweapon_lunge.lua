...
local i={}
local j={}for k=0,10 do 
doer:DoTaskInTime(FRAMES*math.ceil(1+k/3.5),
function()local l=(targetpos-startingpos):GetNormalized()*k*0.6;SpawnPrefab("spear_gungnir_lungefx").Transform:SetPosition((startingpos+l):Get())
...
i[r]=true;
table.insert(j,r)end end;
if k==10 then 
if self.inst.components.weapon and self.inst.components.weapon:HasAltAttack()then 
self.inst.components.weapon:DoAltAttack(doer,j,nil,self.stimuli)end end end)
end;
