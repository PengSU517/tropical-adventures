...
function self:Kill(reset)
    ...
    local function SpawnPenguin(inst, spawner, colonyNum, pos, angle)
        ...
        local pengu
        local numerador = math.random(1, 3)
        if numerador == 1 then
            pengu = SpawnPrefab("mosquito_poison")
        elseif numerador == 2 then
            pengu = SpawnPrefab("mosquito_poison")
        elseif numerador == 3 then
            pengu = SpawnPrefab("mosquito")
        end
    ...
    local function SpawnFlock(colonyNum, loc, check_angle)
        ...
        while spawned < flock and i < flock + 7 do
            ...
            if map:IsAboveGroundAtPoint(spawnPos:Get()) then
                ...
                --print(TheCamera:GetHeading()," spawnPenguin at",pos,"angle:",angle)
                self.inst:DoTaskInTime(GetRandomWithVariance(1, 1), SpawnPenguin, self, colonyNum, spawnPos,
                    (check_angle / DEGREES))
            end
        end
    end
    ...
    local function EstablishColony(loc)
        ...
        if newFlock.rookery then
            ...
            newFlock.ice = SpawnPrefab("flood_ice")
                ...
                while not foundvalidplacement do
                    ...
                    if offset then
                        ...
                        if #ents == 0 then
                            ...
                            --                        local icerock = SpawnPrefab("sedimentpuddle")
    ...
    local function TryToSpawnFlockForPlayer(playerdata)
        if _active then
            ...
            if not TheWorld.state.isspring or TheWorld.state.remainingdaysinseason <= 3 then
                return
            end
            ...
            if player and player.components.areaaware and not player.components.areaaware:CurrentlyInTag("tropical") then return end
    ...
    local function OnLoadColonies(data)
        ...
        if data.colonies then
            for i, v in ipairs(data.colonies) do
                local ice = SpawnPrefab("flood_ice")
                if ice then
