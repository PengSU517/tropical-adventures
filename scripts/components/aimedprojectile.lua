...
function a:SetHitDistance(c) self.hitdist = c end

function a:SetRange(d) self.range = d end

function a:SetSpeed(e) self.speed = e end

function a:SetDamage(f) self.damage = f end

function a:SetStimuli(g) self.stimuli = g end

function a:SetOnThrownFn(h) self.onthrown = h end

function a:SetOnHitFn(h) self.onhit = h end

function a:SetOnMissFn(h) self.onmiss = h end
...
function a:Throw(l, m, n)
    ...
    if m ~= nil and self.launchoffset ~= nil then
        ...
        self.inst.Transform:SetPosition(o +
            self.launchoffset.x * math.cos(r), p + self.launchoffset.y, q - self.launchoffset.x * math.sin(r))
    end
    ...
    if self.onthrown ~= nil then self.onthrown(self.inst, l, m, n) end
end
...
function a:Miss()
    if self.onmiss ~= nil then self.onmiss(self.inst, self.owner, self.attacker) end
...
function a:Hit(s)
    ...
    if self.attacker and self.owner and self.owner.components.weapon then
        self.owner.components.weapon:DoAltAttack(
            self.attacker, s, nil, self.stimuli)
    end

    if self.onhit ~= nil then self.onhit(self.inst, self.owner, self.attacker, s) end
    ...
    if self.range ~= nil and distsq(self.start, u) > self.range * self.range then
        ...
        for x, y in ipairs(w) do
            if y.entity:IsValid() and y.entity:IsVisible() and (y.components.health and not y.components.health:IsDead() or self.inst.prefab == 'riledlucy' and y:HasTag("tree") and not y:HasTag("stump") and y.components.workable and y.components.workable:CanBeWorked() and y.components.workable:GetWorkAction()) then
                ...
                if z > A then table.insert(v, { target = y, hitrange = z, currentrange = A }) end
            end
        end
        ...
        for x, B in pairs(v) do
            if s == nil or B.currentrange - B.hitrange < s.range then
                s = {
                    ent = B.target,
                    range = B.currentrange - B.hitrange
                }
            end
        end

        if s ~= nil then self:Hit(s.ent) end
    end
end
...
function a:DelayVisibility(D)
    if self.delaytask ~= nil then self.delaytask:Cancel() end
