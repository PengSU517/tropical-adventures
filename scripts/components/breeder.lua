...
local FISH_FARM_LURE_TEST_TIME = 30 * 7
...
function Breeder:summonpredator()
    ...
    if spawn_pt then
        ...
        if predator then
            ...
            --联机服务端ThePlayer恒nil
            --ThePlayer is nil on server side
            --[[
            if self.inst:GetDistanceSqToInst(player) < radius * radius then
                offset = Vector3(radius * math.cos(theta), 0, -radius * math.sin(theta))
                base = Vector3(player.Transform:GetWorldPosition())
            end
            --]]
            --或可以域内取玩家代之 IsAnyPlayerInRange(x, y, z, range, isalive)
            --searching players in the area instead
            local players = FindPlayersInRange(base.x, base.y, base.z, radius, true)
            if #players > 0 then
                offset = Vector3(radius * math.cos(theta), 0, -radius * math.sin(theta))
                base = Vector3(players[1].Transform:GetWorldPosition())
            end

            predator.Physics:Teleport(base.x + offset.x, base.y + offset.y, base.z + offset.z)
            predator.components.combat:SuggestTarget(players[1])
        end
    end
end
...
function Breeder:Seed(item)
    ...
    local tipo_de_eixe = math.random(1, 8)
    if tipo_de_eixe < 5 then
        self.product = "fish2"
    end
...
function Breeder:GetDebugString()
    return "seeded: " ..
        tostring(self.seeded) .. " harvestable: " .. tostring(self.harvestable) .. " volume: " .. tostring(self.volume)
end
