local function onavailable(self)
    if self:IsActive() then
        self.inst:AddTag("room_teleporter")
    else
        self.inst:RemoveTag("room_teleporter")
    end
end
...
local Teleporter = Class(function(self, inst)
        ...
        self.travelcameratime = 0.6
        self.travelarrivetime = 0.5
    ...
    if self.migration_data ~= nil then
        ...
        if data.worldid ~= TheShard:GetShardId() and Shard_IsWorldAvailable(data.worldid) then
            TheWorld:PushEvent("ms_playerdespawnandmigrate",
                { player = doer, portalid = nil, worldid = data.worldid, x = data.x, y = data.y, z = data.z })
            return true
        else
            return false
        end
...
function Teleporter:Teleport(obj)
    if self.targetTeleporter ~= nil then
        ...
        local offset = self.targetTeleporter.components.teleporter ~= nil and
            self.targetTeleporter.components.teleporter.offset or 0
