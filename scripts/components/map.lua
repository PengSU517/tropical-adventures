...
function Map:IsPassableAtPoint(x, y, z, allow_water, exclude_boats)
    ...
    local entities = TheSim:FindEntities(x, y, z, 1.2, { "boat" })
    if entities then
        for i, v in ipairs(entities) do
            if v then return true end
        end
...
function Map:IsGroundTargetBlocked(pt, range)
    ...
    for i, v in ipairs(TheSim:FindEntities(pt.x, 0, pt.z, math.max(DEPLOY_EXTRA_SPACING, MAX_GROUND_TARGET_BLOCKER_RADIUS) + range, nil, nil, BLOCKED_ONEOF_TAGS)) do
        local radius = (v.ground_target_blocker_radius or v._groundhole_outerradius or v:GetPhysicsRadius(0)) +
            (v._groundhole_rangeoverride or range)
...
local function IsNearOther(other, pt, min_spacing_sq)
    ...
    return other:GetDistanceSqToPoint(pt.x, 0, pt.z) <
...
function Map:CanDeployAtPoint(pt, inst, mouseover)
    ...
    return (mouseover == nil or mouseover:HasTag("player") or mouseover:HasTag("walkableplatform") or mouseover:HasTag("walkableperipheral"))
        and
        (self:IsPassableAtPointWithPlatformRadiusBias(x, y, z, false, false, TUNING.BOAT.NO_BUILD_BORDER_RADIUS, true) or is_valid_ground == true)
        and
...
function Map:CanDeployAtPointInWater(pt, inst, mouseover, data)
    ...
    if inst.prefab == "wall_enforcedlimestone_item" then
        return self:IsSurroundedByWater(pt.x, pt.y, pt.z,
            min_distance_from_land + radius)
    end
...
function Map:CanDeployMastAtPoint(pt, inst, mouseover)
    ...
    return (mouseover == nil or mouseover:HasTag("player") or mouseover:HasTag("walkableplatform") or mouseover:HasTag("walkableperipheral"))
        and self:IsPassableAtPointWithPlatformRadiusBias(pt.x, pt.y, pt.z, false, false,
...
function Map:CanDeployWalkablePeripheralAtPoint(pt, inst)
    return self:IsDeployPointClear(pt, nil, inst.replica.inventoryitem:DeploySpacingRadius(), nil, nil, nil,
        WALKABLEPERIPHERAL_DEPLOY_IGNORE_TAGS)
end
...
function Map:CanDeployDockAtPoint(pt, inst, mouseover)
    ...
    local tarsource = TheSim:FindEntities(pt.x, 0, pt.z, 3, "tar source")
    for _, v in ipairs(tarsource) do
        if math.sqrt(v:GetDistanceSqToPoint(pt.x, 0, pt.z)) <= min_distance_from_entities + 2 then
            return false
        end
    end
...
function Map:CanDeployRecipeAtPoint(pt, recipe, rot)
    ...
    if BUILDMODE.WATER == recipe.build_mode then
        ...
        is_valid_ground = not self:IsPassableAtPoint(pt_x, pt_y, pt_z) ------------------水面难道是不可通行
        ...
        is_valid_ground = self:IsPassableAtPointWithPlatformRadiusBias(pt_x, pt_y, pt_z, false, false,
            ...
            self:IsValidTileAtPoint(pt_x, pt_y, pt_z)
    end
