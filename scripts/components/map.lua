...
local DEPLOY_IGNORE_TAGS = { "NOBLOCK", "player", "FX", "INLIMBO", "DECOR", "walkableplatform", "walkableperipheral", "alt_tile", "centerlight", "liberado" }
...
function Map:CanDeployAtPoint(pt, inst, mouseover)    
    ...
    return (mouseover == nil or mouseover:HasTag("player") or mouseover:HasTag("walkableplatform") or mouseover:HasTag("walkableperipheral"))
        and (self:IsPassableAtPointWithPlatformRadiusBias(x,y,z, false, false, TUNING.BOAT.NO_BUILD_BORDER_RADIUS, true) or is_valid_ground == true)
...
if inst.prefab == "wall_enforcedlimestone_item" then return self:IsSurroundedByWater(pt.x, pt.y, pt.z, min_distance_from_land + radius) end		
