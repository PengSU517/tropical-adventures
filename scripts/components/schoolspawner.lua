...
return Class(function(self, inst)
    ...
    local function GetOceanTrawlerChanceModifier(spawnpoint)
        local oceantrawlers = TheSim:FindEntities(spawnpoint.x, spawnpoint.y, spawnpoint.z, TUNING.GNARWAIL_TEST_RADIUS,
            OCEANTRAWLER_TAGS)
    ...
    local function testforgnarwail(comp, spawnpoint, chancemodifier)
        local ents = TheSim:FindEntities(spawnpoint.x, spawnpoint.y, spawnpoint.z, TUNING.GNARWAIL_TEST_RADIUS,
            GNARWAIL_TAGS)
    ...
    local function ScheduleSpawn(player)
        if _scheduledtasks[player] == nil then
            _scheduledtasks[player] = player:DoTaskInTime(
                GetRandomMinMax(TUNING.SCHOOL_SPAWN_DELAY.min, TUNING.SCHOOL_SPAWN_DELAY.max), SpawnSchoolForPlayer,
                ScheduleSpawn)
        end
    end
    ...
    local function PickSchool(spawnpoint)
        if FISH_DATA.school[TheWorld.state.season] ~= nil then
            local school_choices = FISH_DATA.school[TheWorld.state.season]
    ...
    function self:ShouldSpawnANewSchoolForPlayer(player)
        ...
        if percent_ocean > TUNING.SCHOOL_SPAWNER_FISH_OCEAN_PERCENT then
            local num_school_spawn_blockers = #TheSim:FindEntities(pt.x, pt.y, pt.z,
                TUNING.SCHOOL_SPAWNER_FISH_CHECK_RADIUS, FISHSCHOOLSPAWNBLOCKER_TAGS)
            if math.random() < 1 - num_school_spawn_blockers * TUNING.SCHOOL_SPAWNER_BLOCKER_MOD then
                local num_fish = #TheSim:FindEntities(pt.x, pt.y, pt.z, TUNING.SCHOOL_SPAWNER_FISH_CHECK_RADIUS,
                    FISHABLE_MUST_TAGS)
                local r = math.random()
    ...
    function self:SpawnSchool(spawnpoint, target, override_spawn_offset)
        ...
        for i = 1, schoolsize do
            ...
            if offset then
                if count == 0 then
                    ...
                    self.inst:DoTaskInTime(0.1 + math.random() * 1,
                        function() DoSpawnFish(schooldata.prefab, school_spawnpoint + offset, rotation, herd) end)
                end
                count = count + 1
            end
        end
        ...
        if count > 0 then
            ...
            if tile_at_spawnpoint == WORLD_TILES.OCEAN_SWELL or
                tile_at_spawnpoint == WORLD_TILES.OCEAN_MEDIUM or
                tile_at_spawnpoint == WORLD_TILES.OCEAN_DEEP then
