...
return Class(function(self, inst)
	...
	local function IsEligible(player)
		...
		return player:IsValid()
			...
			and not area:CurrentlyInTag("tropical")
			and not area:CurrentlyInTag("hamlet")
			and not area:CurrentlyInTag("frost")
	end
	...
	local function SpawnBearger()
		...
		if timetospawn == nil then
			timetospawn = _worldsettingstimer:StartTimer(BEARGER_TIMERNAME,
				GetRandomWithVariance(spawndelay, spawnrandom))
		elseif timetospawn > spawndelay + spawnrandom then
			timetospawn = _worldsettingstimer:SetTimeLeft(BEARGER_TIMERNAME,
				GetRandomWithVariance(spawndelay, spawnrandom))
