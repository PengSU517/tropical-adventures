...
function GroundPounder:DestroyRing(pt, radius, points, breakobjects, dodamage, pushplatforms, pushinventoryitems, spawnfx,
                                   ents_hit, platforms_hit)
    local getEnts = breakobjects or dodamage or pushinventoryitems
    local map = TheWorld.Map
    if dodamage then
        self.inst.components.combat:EnableAreaDamage(false)
    ...
    if pushplatforms then
        ...
        local platform_ents = TheSim:FindEntities(pt.x, pt.y, pt.z, radius + platform_radius, WALKABLEPLATFORM_TAGS,
            self.noTags)
        local i0
...
local function OnDestroyRing(inst, self, pt, radius, points, breakobjects, dodamage, pushplatforms, pushinventoryitems,
                             spawnfx, ents_hit, platforms_hit)
    self:DestroyRing(pt, radius, points, breakobjects, dodamage, pushplatforms, pushinventoryitems, spawnfx, ents_hit,
        platforms_hit)
end
...
function GroundPounder:GroundPound_Offscreen(position)
    ...
    if self.platformPushingRings > 0 then
        ...
        local platformEnts = TheSim:FindEntities(position.x, position.y, position.z,
            platformPushRadius + TUNING.MAX_WALKABLE_PLATFORM_RADIUS, WALKABLEPLATFORM_TAGS, self.noTags)
        for i, p_ent in ipairs(platform_ents) do
...
function GroundPounder:GetDebugString()
    return string.format(
        "num rings: %d, damage rings: %d, destruction rings: %d, boat pushing rings: %d, inventory pushing rings: %d",
        self.numRings,
        self.damageRings,
