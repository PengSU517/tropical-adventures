		...
		if TheWorld.components.aporkalypse and TheWorld.components.aporkalypse.aporkalypse_active == true then
			...
			for i, node in ipairs(TheWorld.topology.nodes) do
				if TheSim:WorldPointInPoly(x, z, node.poly) then
					...
					elseif node.tags ~= nil and table.contains(node.tags, "frost") then
						_spawndata.base_prefab = "icehound"
						_spawndata.winter_prefab = "icehound"
						_spawndata.summer_prefab = "icehound"
	...
	function self:GetDebugString()
		if _timetoattack > 0 then
			return string.format("%s spawns are coming in %2.2f",
				(_warning and "WARNING") or (_pausesources:Get() and "BLOCKED") or "WAITING", _timetoattack)
		end
