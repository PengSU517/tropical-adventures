...
function getfacespeech(inst)
    ...
    if desc then
        ...
        if TheWorld.state.isaporkalypsenear then
...
function CityPigBrain:OnStart()
    ...
    local night = WhileNode(function() return not TheWorld.state.isday end, "IsNight",
        PriorityNode {
            ...
            IfNode(function() return not self.inst:HasTag("guard") and not (TheWorld.state.isfiesta) end, "gohome",
        ...
        PriorityNode(
            {
                ...
                IfNode(function() return (TheWorld.state.isaporkalypse) end, "gohome",
                    ChattyNode(self.inst, getSpeechType(self.inst, STRINGS.CITY_PIG_TALK_GO_HOME),
                        DoAction(self.inst, GoHomeAction, "go home", true))),
