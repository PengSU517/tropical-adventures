...
local function GetHomePos(inst)
    if inst.home_pos then return inst.home_pos end
...
local function CanSummon(inst)
    return not inst.components.health:IsDead() and GetTime() - inst.summon_time > SUMMON_COOLDOWN and
               (inst.components.combat.target and inst.components.combat.target:HasTag("player")) and
               ShoudSummonEntities(inst)
end
...
local function CanTaunt(inst)
    return not inst.components.health:IsDead() and GetTime() - inst.taunt_time > TAUNT_COOLDOWN and
               (inst.components.combat.target and inst.components.combat.target:HasTag("player")) and math.random() <
               0.1
end
...
local AncientHeraldBrain = Class(Brain, function(self, inst) Brain._ctor(self, inst) end)

function AncientHeraldBrain:OnStart()

    local root = PriorityNode({IfNode(function() return CanTaunt(self.inst) end, "CanTaunt",
    ...
    end, "AttackMomentarily", ChaseAndAttack(self.inst, CHASE_TIME, CHASE_DIST)),
                               Wander(self.inst, GetHomePos, CHASE_DIST)}, 1)
