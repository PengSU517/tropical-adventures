...
-------------------------------------------------------------------------------------------------------
local WikibookPage = Class(Widget, function(self, parent_screen, category)
    Widget._ctor(self, "WikibookPage")
...
function WikibookPage:_DoFocusHookups()
...
function WikibookPage:CreateRecipeBook()
    ...
    local details_decor = details_decor_root:AddChild(Image("images/quagmire_recipebook.xml",
        "quagmire_recipe_menu_block.tex"))
    details_decor:ScaleToSize(360, 500)
    details_decor = details_decor_root:AddChild(Image("images/quagmire_recipebook.xml",
        "quagmire_recipe_corner_decoration.tex"))
    ...
    local dis_y = 238
    dis_y = dis_y - 18 / 2
    local completed = panel_root:AddChild(Text(HEADERFONT, 18, STRINGS.UI.COOKBOOK.DISCOVERED_RECIPES,
    ...
    dis_y = dis_y - 10
    dis_y = dis_y - 18 / 2
    completed = panel_root:AddChild(Text(HEADERFONT, 18, subfmt(STRINGS.UI.XPUTILS.XPPROGRESS, {
        num = self.num_recipes_discovered,
        max = #self.all_recipes
    }), UICOLOURS.BROWN_DARK))
    completed:SetHAlign(ANCHOR_RIGHT)
    completed:SetPosition(dis_x, dis_y)
    dis_y = dis_y - 18 / 2

    self.details_root:AddChild(self:PopulateRecipeDetailPanel(
        self.all_recipes[(TheCookbook.selected ~= nil and TheCookbook.selected[self.category] or 1)]))
...
function WikibookPage:_SetupRecipeIngredientDetails(recipes, parent, y)
    ...
    if #recipes <= 3 then
        for b = 1, #recipes do
            ...
            for i = 1, #items do
                local backing =
                    inv_backing_root:AddChild(Image("images/quagmire_recipebook.xml", "ingredient_slot.tex"))
                backing:ScaleToSize(ingredient_size, ingredient_size)
                backing:SetPosition(x + (i) * ingredient_size + (i - 1) * x_spacing,
                    y - ingredient_size / 2 - (b - 1) * (ingredient_size + 5))
                ...
                img:SetHoverText(STRINGS.NAMES[string.upper(ingredient_name_remap[items[i]] or items[i])] or
                                     subfmt(STRINGS.UI.COOKBOOK.UNKNOWN_INGREDIENT_NAME, {
                        ingredient = items[i]
                    }))
            end
            index = index + 1
        ...
        for b = 1, #recipes do
            ...
            for i = 1, #items do
                ...
                backing:ScaleToSize(ingredient_size, ingredient_size)
                backing:SetPosition(x + (i) * ingredient_size + (i - 1) * x_spacing,
                    y - ingredient_size / 2 - math.floor((b - 1) / 2) * (ingredient_size + 5))
...
function WikibookPage:_GetSpoilString(perishtime)
    return perishtime == nil and STRINGS.UI.COOKBOOK.PERISH_NEVER or perishtime <= TUNING.PERISH_SUPERFAST and
               STRINGS.UI.COOKBOOK.PERISH_VERY_QUICKLY or perishtime <= TUNING.PERISH_FAST and
               STRINGS.UI.COOKBOOK.PERISH_QUICKLY or perishtime <= TUNING.PERISH_MED and
               STRINGS.UI.COOKBOOK.PERISH_AVERAGE or perishtime <= TUNING.PERISH_SLOW and
               STRINGS.UI.COOKBOOK.PERISH_SLOWLY or STRINGS.UI.COOKBOOK.PERISH_VERY_SLOWLY
end

function WikibookPage:_GetCookingTimeString(cooktime)
    return cooktime == nil and STRINGS.UI.COOKBOOK.COOKINGTIME_UNKNOWN or cooktime < 1.0 and
               STRINGS.UI.COOKBOOK.COOKINGTIME_SHORT or cooktime < 2.0 and STRINGS.UI.COOKBOOK.COOKINGTIME_AVERAGE or
               cooktime < 2.5 and STRINGS.UI.COOKBOOK.COOKINGTIME_LONG or STRINGS.UI.COOKBOOK.COOKINGTIME_VERY_LONG
end

function WikibookPage:_GetSideEffectString(recipe_def)
    return recipe_def.oneat_desc or (recipe_def.temperature ~= nil and recipe_def.temperature > 0) and
               STRINGS.UI.COOKBOOK.FOOD_EFFECTS_HOT_FOOD or
               (recipe_def.temperature ~= nil and recipe_def.temperature < 0) and
               STRINGS.UI.COOKBOOK.FOOD_EFFECTS_COLD_FOOD or STRINGS.UI.COOKBOOK.FOOD_EFFECTS_NONE
end

function WikibookPage:PopulateRecipeDetailPanel(data)
    ...
    local title = details_root:AddChild(Text(HEADERFONT, name_font_size,
        data.unlocked and data.name or STRINGS.UI.RECIPE_BOOK.UNKNOWN_RECIPE, UICOLOURS.BROWN_DARK))
    ...
    if not data.unlocked then
        ...
        msg:SetMultilineTruncatedString(STRINGS.UI.COOKBOOK.LOCKED_RECIPE[self.category] or
                                            STRINGS.UI.COOKBOOK.LOCKED_RECIPE.COOKPOT, 20, 300)
        ...
        local food_img = portrait_root:AddChild(Image(data.food_atlas,
            not data.unlocked and "cookbook_unknown.tex" or data.food_tex))
        ...
        if data.has_eaten then
            ...
            self.health_status = details_root:AddChild(TEMPLATES.MakeUIStatusBadge(
                (health ~= nil and health >= 0) and "health" or "health_bad"))
            ...
            self.hunger_status = details_root:AddChild(TEMPLATES.MakeUIStatusBadge(
                (hunger ~= nil and hunger >= 0) and "hunger" or "hunger_bad"))
            ...
            self.sanity_status = details_root:AddChild(TEMPLATES.MakeUIStatusBadge(
                (sanity ~= nil and sanity >= 0) and "sanity" or "sanity_bad"))
            ...
            if effects_str then
                ...
                title = details_root:AddChild(Text(HEADERFONT, value_title_font_size,
                    STRINGS.UI.COOKBOOK.FOOD_EFFECTS_TITLE, UICOLOURS.BROWN_DARK))
        ...
        title = details_root:AddChild(Text(HEADERFONT, title_font_size, STRINGS.UI.COOKBOOK.FOOD_TYPE_TITLE,
            UICOLOURS.BROWN_DARK))
        ...
        local str = STRINGS.UI.FOOD_TYPES[data.recipe_def.foodtype or FOODTYPE.GENERIC] or
                        STRINGS.UI.COOKBOOK.FOOD_TYPE_UNKNOWN
        ...
        -- Perish Rate
        y = y - title_font_size / 2
        title = details_root:AddChild(Text(HEADERFONT, title_font_size, STRINGS.UI.COOKBOOK.PERISH_RATE_TITLE,
            UICOLOURS.BROWN_DARK))
        title:SetPosition(column_offset_x, y)
        y = y - title_font_size / 2
        MakeDetailsLine(details_root, column_offset_x, y - 2, .5, "quagmire_recipe_line_veryshort.tex")
        ...
        if data.recipes ~= nil and #data.recipes > 0 then
            -- Cooking Time
            y = y - title_font_size / 2
            title = details_root:AddChild(Text(HEADERFONT, title_font_size, STRINGS.UI.COOKBOOK.COOKINGTIME_TITLE,
                UICOLOURS.BROWN_DARK))
            title:SetPosition(0, y)
            y = y - title_font_size / 2
            ...
            -- INGREDIENTS
            y = y - title_font_size / 2
            title = details_root:AddChild(Text(HEADERFONT, title_font_size,
                STRINGS.UI.RECIPE_BOOK.DETAILS_LABEL_RECIPES, UICOLOURS.BROWN_DARK))
            title:SetPosition(0, y)
            y = y - title_font_size / 2
            MakeDetailsLine(details_root, 0, y - 2, .49)
            y = y - 10
            ...
            y = y - title_font_size / 2
            title = details_root:AddChild(Text(HEADERFONT, title_font_size, STRINGS.UI.COOKBOOK.NO_RECIPES_TITLE,
                UICOLOURS.BROWN_DARK))
            title:SetPosition(0, y)
            y = y - title_font_size / 2
            MakeDetailsLine(details_root, 0, y - 2, .49)
...
function WikibookPage:BuildRecipeBook()
    local base_size = 128
    local cell_size = 73
    ...
    local function ScrollWidgetsCtor(context, index)
        ...
        ----------------
        w.cell_root = w:AddChild(ImageButton("images/quagmire_recipebook.xml", "cookbook_unknown.tex",
        ...
        w.partiallyknown_icon = w.recipie_root:AddChild(Image("images/quagmire_recipebook.xml",
            "cookbook_unknown_icon.tex"))
    ...
    local function ScrollWidgetSetData(context, widget, data, index)
        ...
        if data ~= nil then
            ...
            if data.unlocked then
                ...
                widget.food_img:SetTexture(data.food_atlas,
                    not data.unlocked and "cookbook_unknown.tex" or data.food_tex)
                ...
                widget.cell_root:SetTextures("images/quagmire_recipebook.xml", "cookbook_unknown.tex",
                    "cookbook_unknown_selected.tex")
            end
            widget:Enable()
        else
    ...
    for prefab, recipe_def in pairs(cookbook_recipes) do
        ...
        local data = {
            prefab = prefab,
            name = STRINGS.NAMES[string.upper(prefab)] or subfmt(STRINGS.UI.COOKBOOK.UNKNOWN_FOOD_NAME, {
                food = prefab or "SDF"
            }),
...
function WikibookPage:_sortfn_default(a, b)
    return a.recipe_def.priority > b.recipe_def.priority or
               (a.recipe_def.priority == b.recipe_def.priority and a.defaultsortkey > b.defaultsortkey)
end

function WikibookPage:_sortfn_sideeffects(a, b)
    local a_score = not a.unlocked and 0 or not a.has_eaten and 1 or
                        (a.recipe_def.oneat_desc == nil and a.recipe_def.temperature == nil) and 2 or 3

    local b_score = not b.unlocked and 0 or not b.has_eaten and 1 or
                        (b.recipe_def.oneat_desc == nil and b.recipe_def.temperature == nil) and 2 or 3
...
function WikibookPage:ApplySort()
    local sortby = TheCookbook:GetFilter("sort")
    table.sort(self.filtered_recipes, sortby == "alphabetical" and function(a, b)
        return a.unlocked and not b.unlocked or (a.unlocked and b.unlocked and a.name < b.name)
    end or sortby == "health" and function(a, b)
        return (a.unlocked and not b.unlocked) or (a.has_eaten and not b.has_eaten) or (a.has_eaten and
                   ((a.recipe_def.health > b.recipe_def.health) or
                       (a.recipe_def.health == b.recipe_def.health and a.name < b.name)))
    end or sortby == "hunger" and function(a, b)
        return (a.unlocked and not b.unlocked) or (a.has_eaten and not b.has_eaten) or (a.has_eaten and
                   ((a.recipe_def.hunger > b.recipe_def.hunger) or
                       (a.recipe_def.hunger == b.recipe_def.hunger and a.name < b.name)))
    end or sortby == "sanity" and function(a, b)
        return (a.unlocked and not b.unlocked) or (a.has_eaten and not b.has_eaten) or (a.has_eaten and
                   ((a.recipe_def.sanity > b.recipe_def.sanity) or
                       (a.recipe_def.sanity == b.recipe_def.sanity and a.name < b.name)))
    end or sortby == "sideeffects" and function(a, b)
        return self:_sortfn_sideeffects(a, b)
    end or function(a, b)
        return self:_sortfn_default(a, b)
    end)
...
function WikibookPage:ApplyFilters()
    ...
    for i, item in ipairs(self.all_recipes) do
        ...
        if (filterby == FILTER_ALL) or (filterby == FOODTYPE.MEAT and foodtype == FOODTYPE.MEAT) or
            (filterby == FOODTYPE.VEGGIE and foodtype == FOODTYPE.VEGGIE) or
            (filterby == "OTHER" and foodtype ~= FOODTYPE.MEAT and foodtype ~= FOODTYPE.VEGGIE) or
            (filterby == "SIDEEFFECTS" and (item.recipe_def.oneat_desc ~= nil or item.recipe_def.temperature ~= nil)) or
            (filterby == "INCOMPLETE" and (item.recipes == nil or not item.has_eaten)) then
...
function WikibookPage:BuildSpinners()
    ...
    local sort_options = {{
        text = STRINGS.UI.COOKBOOK.SORT_DEFAULT,
        data = "default"
    }, {
        text = STRINGS.UI.COOKBOOK.SORT_ALPHABETICAL,
        data = "alphabetical"
    }, {
        text = STRINGS.UI.COOKBOOK.SORT_HEALTH,
        data = "health"
    }, {
        text = STRINGS.UI.COOKBOOK.SORT_HUNGER,
        data = "hunger"
    }, {
        text = STRINGS.UI.COOKBOOK.SORT_SANITY,
        data = "sanity"
    }, {
        text = STRINGS.UI.COOKBOOK.SORT_SIDE_EFFECTS,
        data = "sideeffects"
    }}
    ...
    local filter_options = {{
        text = STRINGS.UI.COOKBOOK.FILTER_ALL,
        data = FILTER_ALL
    }, {
        text = STRINGS.UI.COOKBOOK.FILTER_MEAT,
        data = FOODTYPE.MEAT
    }, {
        text = STRINGS.UI.COOKBOOK.FILTER_VEGGIE,
        data = FOODTYPE.VEGGIE
    }, {
        text = STRINGS.UI.COOKBOOK.FILTER_OTHER,
        data = "OTHER"
    }, {
        text = STRINGS.UI.COOKBOOK.FILTER_SIDE_EFFECTS,
        data = "SIDEEFFECTS"
    }, {
        text = STRINGS.UI.COOKBOOK.FILTER_INCOMPLETE,
        data = "INCOMPLETE"
    }}
    ...
    local function MakeSpinner(labeltext, spinnerdata, onchanged_fn, initial_data)
        ...
        local lean = true
        wdg.spinner = wdg:AddChild(Spinner(spinnerdata, width_spinner, height, {
            font = font,
            size = font_size
        }, nil, "images/quagmire_recipebook.xml", nil, lean))
    ...
    local items = {}
    table.insert(items, MakeSpinner(STRINGS.UI.COOKBOOK.SORT_SPINNERLABEL, sort_options, on_sort_fn,
        TheCookbook:GetFilter("sort")))
    table.insert(items, MakeSpinner(STRINGS.UI.COOKBOOK.FILTER_SPINNERLABEL, filter_options, on_filter_fn,
        TheCookbook:GetFilter("filter")))
