    ...
    if _G.TUNING.FORGE_ITEM_PACK[c] and _G.TUNING.FORGE_ITEM_PACK[c].DURABILITY and _G.TUNING.FORGE_ITEM_PACK[c].DURABILITY.AMOUNT then
        if _G.TUNING.FORGE_ITEM_PACK[c].DURABILITY.AMOUNT > 0 then
            ...
            if _G.TUNING.FORGE_ITEM_PACK[c].DURABILITY.CONSUMPTION_NORMAL then
                b.components.finiteuses:SetConsumption(ACTIONS.ATTACK,
...
local function e(b)
    local c = b.prefab:upper()
    if _G.TUNING.FORGE_ITEM_PACK[c] and _G.TUNING.FORGE_ITEM_PACK[c].DURABILITY then
        if _G.TUNING.FORGE_ITEM_PACK[c].DURABILITY > 0 then
            b:AddComponent("fueled")
            b.components.fueled.fueltype = FUELTYPE.USAGE;
            b.components.fueled:InitializeFuelLevel(_G.TUNING.FORGE_ITEM_PACK[c].DURABILITY * _G.TUNING.TOTAL_DAY_TIME)
            b.components.fueled:SetDepletedFn(b.Remove)
        end
    else
        print("Cannot find tunning for " .. c)
    end
end;
local f = { "tfwp_lava_dart", "tfwp_lava_hammer", "tfwp_spear_gung", "tfwp_healing_staff", "tfwp_dragon_dart",
    "tfwp_infernal_staff", "tfwp_spear_lance", "tfwp_fire_bomb", "tfwp_heavy_sword", "tfwp_control_book",
    "tfwp_summon_book" }
