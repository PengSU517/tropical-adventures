...
local Aporkalypse = Class(function(self, inst)
	...
	self._phase = net_tinybyte(inst.GUID, "aporkalypse.self._phase", "aporkalypsephasedirty")
	self._phase:set(PHASES.calm)

```

如毁灭季组件，这里最重要的是定义了一个网络变量`self._phase = net_tinybyte(inst.GUID, "aporkalypse.self._phase", "aporkalypsephasedirty")`用来存储毁灭季的阶段。
