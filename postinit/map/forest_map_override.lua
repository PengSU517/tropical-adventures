...
local function Generate(prefab, map_width, map_height, tasks, level, level_type)
    ...
    if current_gen_params.start_location ~= nil then
        ...
        story_gen_params.start_setpeice = type(start_loc.start_setpeice) == "table" and
            start_loc.start_setpeice[math.random(#start_loc.start_setpeice)] or start_loc.start_setpeice
        story_gen_params.start_node = type(start_loc.start_node) == "table" and
            start_loc.start_node[math.random(#start_loc.start_node)] or start_loc.start_node
        if story_gen_params.start_node == nil then
            -- existing_start_node is no longer supported
            story_gen_params.start_node = type(start_loc.existing_start_node) == "table" and
                start_loc.existing_start_node[math.random(#start_loc.existing_start_node)] or
                start_loc.existing_start_node
        end
    end
    ...
    local add_fn = {
        fn = function(prefab, points_x, points_y, current_pos_idx, entitiesOut, width, height, prefab_list, prefab_data,
                      rand_offset)
    ...
    if topology_save.GlobalTags["Labyrinth"] ~= nil and GetTableSize(topology_save.GlobalTags["Labyrinth"]) > 0 then
        for task, labyrinth_nodes in pairs(topology_save.GlobalTags["Labyrinth"]) do
            ...
            local xs, ys, types = WorldSim:RunMaze(mazetype, val, WORLD_TILES.IMPASSABLE, WORLD_TILES.BRICK,
                labyrinth_nodes)
            ...
            if xs ~= nil and #xs > 0 then
                for idx = 1, #xs do
                    if types[idx] == 0 then
                        ...
                        local x = (xs[idx] + 1.5 - map_width / 2.0) *
                            TILE_SCALE --gjans: note the +1.5 instead of +0.5... RunMaze points are in a strange position.
            ...
            for i, node in ipairs(topology_save.GlobalTags["LabyrinthEntrance"][task]) do
                ...
                for id, edge in pairs(entrance_node.edges) do
                    if edge.node1.data.type ~= NODE_TYPE.Blank and edge.node2.data.type ~= NODE_TYPE.Blank then
                        WorldSim:DrawCellLine(edge.node1.id, edge.node2.id, NODE_INTERNAL_CONNECTION_TYPE.EdgeSite,
                            WORLD_TILES.BRICK)
                    end
                end
            end
        ...
        for task, nodes in pairs(topology_save.GlobalTags["Maze"]) do
            ...
            if xs ~= nil and #xs > 0 then
                ...
                if choices.archive ~= nil then
                    ...
                    obj_layout.Place({ xs[endidx], ys[endidx] }, MAZE_CELL_EXITS_INV[math.abs(types[endidx])], add_fn,
                        choices.special.finish)

                    choice = math.random(1, #ends)
                    local startidx = ends[choice]
                    obj_layout.Place({ xs[startidx], ys[startidx] }, MAZE_CELL_EXITS_INV[math.abs(types[startidx])],
                        add_fn, choices.special.start)
                    ...
                    if choices.archive.keyroom then
                        obj_layout.Place({ xs[reservedindex], ys[reservedindex] }, "SINGLE_NORTH", add_fn,
                            choices.archive.keyroom)
                    end
                ...
                elseif choices.special ~= nil then
                    ...
                    for idx = 1, #xs do
                        ...
                        elseif types[idx] > 0 then
                            obj_layout.Place({ xs[idx], ys[idx] }, MAZE_CELL_EXITS_INV[types[idx]], add_fn, choices
                                .rooms)
                        else
                            obj_layout.Place({ xs[idx], ys[idx] }, MAZE_CELL_EXITS_INV[-types[idx]], add_fn,
                                choices.bosses)
                    ...
                    local idx = ends[1]
                    obj_layout.Place({ xs[idx], ys[idx] }, MAZE_CELL_EXITS_INV[math.abs(types[idx])], add_fn,
                        choices.special.start)
                    idx = ends[#ends]
                    obj_layout.Place({ xs[idx], ys[idx] }, MAZE_CELL_EXITS_INV[math.abs(types[idx])], add_fn,
                        choices.special.finish)
                else
                ...
                for i, node in ipairs(topology_save.GlobalTags["MazeEntrance"][task]) do
                    ...
                    for id, edge in pairs(entrance_node.edges) do
                        if edge.node1.data.type ~= NODE_TYPE.Blank and edge.node2.data.type ~= NODE_TYPE.Blank then
                            WorldSim:DrawCellLine(edge.node1.id, edge.node2.id, NODE_INTERNAL_CONNECTION_TYPE.EdgeSite,
                                choices.bridge_ground or WORLD_TILES.BRICK, nil, true)
                            ...
                            if table.contains(othernode.data.tags, "ForceDisconnected") then
                                for id, edge in pairs(othernode.edges) do
                                    if edge.node1.data.type ~= NODE_TYPE.Blank and edge.node2.data.type ~= NODE_TYPE.Blank then
                                        WorldSim:DrawCellLine(edge.node1.id, edge.node2.id,
                                            NODE_INTERNAL_CONNECTION_TYPE.EdgeSite, WORLD_TILES.FAKE_GROUND, nil, true)
                                    end
                                end
                            end
                        end
    ...
    topology_save.root:PopulateVoronoi(SpawnFunctions, entities, map_width, map_height, translated_prefabs,
        save.map.generated.densities)
    if story_gen_params.has_ocean then
        local ocean_gen_config = require("map/ocean_gen_config")
        Ocean_SetWorldForOceanGen(WorldSim)
        Ocean_PlaceSetPieces(level.ocean_prefill_setpieces, add_fn, obj_layout, WORLD_TILES.IMPASSABLE,
            ocean_gen_config.ocean_prefill_setpieces_min_land_dist, save.map.topology, map_width, map_height)
        ...
        PopulateOcean(SpawnFunctions, entities, map_width, map_height, storygen.ocean_population, translated_prefabs,
            ocean_gen_config.ocean_prefill_setpieces_min_land_dist, save.map.topology)
    ...
    for prefab, count in pairs(double_check) do
        print("Checking Required Prefab " ..
            prefab ..
            " has at least " ..
            count .. " instances (" .. (entities[prefab] ~= nil and #entities[prefab] or 0) .. " found).")

        if entities[prefab] == nil or #entities[prefab] < count then
            if level.overrides[prefab] == "never" then
                print(string.format(" - missing required prefab [%s] was disabled in the world generation options!",
                    prefab))
            else
                print(string.format("PANIC: missing required prefab [%s]! Expected %d, got %d", prefab, count,
                    entities[prefab] == nil and 0 or #entities[prefab]))
                if SKIP_GEN_CHECKS == false then
                    return nil
                end
            end
    ...
    save.map.tiles, save.map.tiledata, save.map.nav, save.map.adj, save.map.nodeidtilemap = WorldSim:GetEncodedMap(
        join_islands)
    ...
    build_porkland(save.ents, TOPOLOGY_SAVE, map_width, map_height, deepcopy(level.overrides))
    -------------------------------------------------------------------------------------------------------------------
